...
{assign var="uitype" value=$maindata[0][0]}
		...
		{if $uitype eq '10'}
			<td width=20% class="dvtCellLabel" align=right>
				...
				{if $fromlink eq 'qcreate'}
					<select id="{$fldname}_type" class="slds-select" name="{$fldname}_type" onChange='document.QcEditView.{$fldname}_display.value=""; document.QcEditView.{$fldname}.value="";'>
				{else}
					<select id="{$fldname}_type" class="slds-select" name="{$fldname}_type" onChange='document.EditView.{$fldname}_display.value=""; document.EditView.{$fldname}.value="";$("qcform").innerHTML=""'>
				{/if}
				...
				{if $MASS_EDIT eq '1'}
					<span class="slds-checkbox">
						<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check">
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				{if $fldname eq 'related_to' && $MODULE eq 'Potentials'}
					<input class="slds-input" id="{$fldname}_display" name="{$fldname}_display" id="edit_{$fldname}_display" onkeyup="if (this.value=='' || this.value.length==1) this.form.{$fldname}.value='';" type="text"  value="{$fldvalue.displayvalue}">&nbsp;
				{else}
					<input class="slds-input" id="{$fldname}_display" name="{$fldname}_display" id="edit_{$fldname}_display" readonly type="text"  value="{$fldvalue.displayvalue}">&nbsp;
				{/if}
				{if $fromlink eq 'qcreate'}
					<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" tabindex="{$vt_tab}" alt="Select" title="Select" LANGUAGE=javascript  onclick='return window.open("index.php?module="+ document.QcEditView.{$fldname}_type.value +"&action=Popup&html=Popup_picker&form=vtlibPopupView&forfield={$fldname}&srcmodule={$MODULE}&forrecord={$ID}","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;
				{else}
					<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" tabindex="{$vt_tab}" alt="Select" title="Select" LANGUAGE=javascript  onclick='return window.open("index.php?module="+ document.EditView.{$fldname}_type.value +"&action=Popup&html=Popup_picker&form=vtlibPopupView&forfield={$fldname}&srcmodule={$MODULE}&forrecord={$ID}","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;
				{/if}
					<input type="image" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="Clear" title="Clear" LANGUAGE=javascript	onClick="this.form.{$fldname}.value=''; this.form.{$fldname}_display.value=''; return false;" align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;
			</td>
		{* END *}
		{elseif $uitype eq 2}
			...
			<td ng-show ="show_logic('{$fldname}')" width=30% align=left class="dvtCellInfo">
				{if $uitype eq 2 && $MODULE eq 'Potentials' && $fldname eq 'potentialname'}
					<input  {$uitype} type="text" name="{$fldname}" disabled tabindex="{$vt_tab}" value="{$smarty.now|date_format:"%Y-%m"}" tabindex="{$vt_tab}" class="slds-input">
				{else}
					<input ng-disabled ="!editable_logic('{$fldname}')" type="text" name="{$fldname}" tabindex="{$vt_tab}" value="{$fldvalue}" tabindex="{$vt_tab}" class="slds-input">
				{/if}
			</td>
		{elseif $uitype eq 3 || $uitype eq 4}<!-- Non Editable field, only configured value will be loaded -->
			...
			<td width=30% align=left class="dvtCellInfo">
				<input readonly type="text" tabindex="{$vt_tab}" name="{$fldname}" id ="{$fldname}" {if $MODE eq 'edit'} value="{$fldvalue}" {else} value="{$MOD_SEQ_ID}" {/if} class="slds-input">
			</td>
		{elseif $uitype eq 11 || $uitype eq 1 || $uitype eq 13 || $uitype eq 7}	
			<td ng-show ="show_logic('{$fldname}')" width=20% class="dvtCellLabel" align=right><font color="red">{$mandatory_field}</font>{$usefldlabel}
				{if $MASS_EDIT eq '1'}
					<span class="slds-checkbox">
						<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check">
							<label class="slds-checkbox__label" for="{$fldname}_mass_edit_check">
			...
			{if $fldname eq 'tickersymbol' && $MODULE eq 'Accounts'}
				<td width=30% align=left class="dvtCellInfo">
					...
					<span id="vtbusy_info" style="display:none;"><img src="{'vtbusy.gif'|@vtiger_imageurl:$THEME}" border="0"></span>
				</td>
			{elseif $fldname eq 'cal_color'}
				<td width=30% align=left class="dvtCellInfo">
					<input type="text" name="{$fldname}" tabindex="{$vt_tab}" id ="{$fldname}" value="{$fldvalue}" class="slds-input" style="width:65px; background-color: {$fldvalue}" >
					&nbsp;
			...
			{else}
				<td ng-show ="show_logic('{$fldname}')" width=30% align=left class="dvtCellInfo">
					{if $MODULE eq 'Settings' && $fldname eq 'gmailpass2'}
						<input type="password" tabindex="{$vt_tab}" name="{$fldname}" id ="{$fldname}" value="{$fldvalue}" class="slds-input">
					{else}
						<input ng-disabled ="!editable_logic('{$fldname}')" type="text" tabindex="{$vt_tab}" name="{$fldname}" id ="{$fldname}" value="{$fldvalue}" class="slds-input">
					{/if}
				</td>
			{/if}
		{elseif $uitype eq 9}
			<td width=20% class="dvtCellLabel" align=right><font color="red">{$mandatory_field}</font>{$usefldlabel} {$APP.COVERED_PERCENTAGE}
				{if $MASS_EDIT eq '1'}
					<span class="slds-checkbox">
						<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check">
							<label class="slds-checkbox__label" for="{$fldname}_mass_edit_check">
			...
			<td width=30% align=left class="dvtCellInfo"><input type="text" tabindex="{$vt_tab}" name="{$fldname}" id ="{$fldname}" value="{$fldvalue}" class=detailedViewTextBox ></td>
		{elseif $uitype eq 19 || $uitype eq 20}<!-- In Add Comment are we should not display anything -->
			{if $fldlabel eq $MOD.LBL_ADD_COMMENT}
				{assign var=fldvalue value=""}
			{/if}
			{if $header eq 'Categorie prodotti' and $MODULE eq 'Potentials' and $MODE eq 'edit'}
				<tr>
					<td colspan=4>
						<div style="width:auto;display:block;" id="tblAdocdetails" >
							<div style="width:auto;display:block;" id="tblAdocdetails" >{include file="EditViewUIKendo.tpl"}</div>
						</div>
					</td>
				</tr>
				<br/>
			...
			<tr>
				...
				<td colspan=3>
					<textarea tets class="slds-textarea" tabindex="{$vt_tab}" name="{$fldname}" cols="90" rows="8">{$fldvalue}</textarea>
		...
		{elseif $uitype eq 33}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<select MULTIPLE name="{$fldname}[]" size="4" tabindex="{$vt_tab}" class="slds-select">
					{foreach item=arr from=$fldvalue}
						<option value="{$arr[1]}" {$arr[2]}>{$arr[0]}</option>
					{/foreach}
				</select>
		...
		{elseif $uitype eq 53}
			<td width="20%" class="dvtCellLabel" align=right ng-show ="show_logic('{$fldname}')">
				<font color="red">{$mandatory_field}</font>{$usefldlabel}
				{if $MASS_EDIT eq '1'}
					<span class="slds-checkbox">
						<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check">
			...
			<td width="30%" align=left class="dvtCellInfo" ng-show ="show_logic('{$fldname}')">
				{assign var=check value=1}
					...
					{/if}
				<input ng-disabled ="!editable_logic('{$fldname}')"  type="radio" tabindex="{$vt_tab}" name="assigntype" {$select_user} value="U" onclick="toggleAssignType(this.value)" >&nbsp;{$APP.LBL_USER}

				{if $secondvalue neq ''}
					<input ng-disabled ="!editable_logic('{$fldname}')" type="radio" name="assigntype" {$select_group} value="T" onclick="toggleAssignType(this.value)">&nbsp;{$APP.LBL_GROUP}
				{/if}

				<span id="assign_user" style="{$style_user}">
					<select name="{$fldname}" class="slds-select" ng-disabled ="!editable_logic('{$fldname}')">
						{foreach key=key_one item=arr from=$fldvalue}
							{foreach key=sel_value item=value from=$arr}
				...
				{if $secondvalue neq ''}
					<span id="assign_team" style="{$style_group}" ng-disabled ="!editable_logic('{$fldname}')">
						<select name="assigned_group_id" class="slds-select">
		...
		{elseif $uitype eq 50}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input readonly name="account_name" type="text" class="slds-input" value="{$fldvalue}">
				<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
				...
				<input type="image" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.account_id.value=''; this.form.account_name.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
			</td>
		{elseif $uitype eq 73}
			<td width="20%" class="dvtCellLabel" align=right>
				<font color="red">{$mandatory_field}</font>{$usefldlabel}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input readonly name="account_name" id="single_accountid" type="text" class="slds-input" value="{$fldvalue}">
				<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
		...
		{elseif $uitype eq 75 || $uitype eq 81}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input name="vendor_name" readonly type="text" class="slds-input" value="{$fldvalue}">
				<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
				<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='return window.open("index.php?module=Vendors&action=Popup&html=Popup_picker&popuptype={$pop_type}&form=EditView&fromlink={$fromlink}","test","width=640,height=602,resizable=0,scrollbars=0");' align="absmiddle" style='width:18px;cursor:pointer;'>
				<input type="image" tabindex="{$vt_tab}" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.vendor_id.value='';this.form.vendor_name.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
			</td>
		{elseif $uitype eq 57}
			<td width="20%" class="dvtCellLabel" align=right>
				<font color="red">{$mandatory_field}</font>{$usefldlabel}
			...
			<td width="30%" align=left class="dvtCellInfo">
				{if $fromlink eq 'qcreate'}
					<input name="contact_name" readonly type="text" class="slds-input" value="{$fldvalue}">
					<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
					<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='selectContact("false","general",document.QcEditView)' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;<input type="image" tabindex="{$vt_tab}" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.contact_id.value=''; this.form.contact_name.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
				{else}
					<input name="contact_name" readonly type="text" class="slds-input" value="{$fldvalue}">
					<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
					<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='selectContact("false","general",document.EditView)' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;<input type="image" tabindex="{$vt_tab}" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.contact_id.value=''; this.form.contact_name.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
				{/if}
			</td>
		{elseif $uitype eq 58}
			<td width="20%" class="dvtCellLabel" align=right>
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input name="campaignname" readonly type="text" class="slds-input" value="{$fldvalue}">
				<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
				<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='return window.open("index.php?module=Campaigns&action=Popup&html=Popup_picker&popuptype=specific_campaign&form=EditView&fromlink={$fromlink}","test","width=640,height=602,resizable=0,scrollbars=0");' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;<input type="image" tabindex="{$vt_tab}" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.campaignid.value=''; this.form.campaignname.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
			</td>
		{elseif $uitype eq 80}
			<td width="20%" class="dvtCellLabel" align=right>
				<font color="red">{$mandatory_field}</font>{$usefldlabel}
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				<input name="{$fldname}" type="hidden" value="{$secondvalue}">&nbsp;
				<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='selectSalesOrder();' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;<input type="image" tabindex="{$vt_tab}" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.salesorder_id.value=''; this.form.salesorder_name.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
			</td>
		{elseif $uitype eq 78}
			<td width="20%" class="dvtCellLabel" align=right>
				<font color="red">{$mandatory_field}</font>{$usefldlabel}
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				<input name="{$fldname}" type="hidden" value="{$secondvalue}" >&nbsp;
				<img src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='selectQuote()' align="absmiddle" style='width:18px;cursor:pointer;' >&nbsp;<input type="image" tabindex="{$vt_tab}" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.quote_id.value=''; this.form.quote_name.value='';return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
			</td>
		{elseif $uitype eq 76}
			<td width="20%" class="dvtCellLabel" align=right>
				<font color="red">{$mandatory_field}</font>{$usefldlabel}
		...
		{elseif $uitype eq 85}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<img src="{'skype.gif'|@vtiger_imageurl:$THEME}" alt="Skype" title="Skype" LANGUAGE=javascript align="absmiddle"/>
			...
			<td width="30%" align=left class="dvtCellInfo">
				{if $fldname eq "unit_price" && $fromlink neq 'qcreate'}
					<span id="multiple_currencies">
						<input name="{$fldname}" id="{$fldname}" tabindex="{$vt_tab}" type="text" class="slds-input" onBlur="updateUnitPrice('unit_price', '{$BASE_CURRENCY}');" value="{$fldvalue}">
						{if $MASS_EDIT neq 1}
								...
								{foreach item=price key=count from=$PRICE_DETAILS}
									<tr>
										...
										<td class="dvtCellInfo" align="center">
											<input {$disable_value} type="button" class="slds-button slds-button--small slds-button--brand" id="cur_reset{$price.curid}"  onclick="updateCurrencyValue(this,'{$price.curname}','{$BASE_CURRENCY}','{$price.conversionrate}');" value="{$APP.LBL_RESET}"/>
										</td>
		...
		{elseif $uitype eq 23 || $uitype eq 5 || $uitype eq 6}
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				{/foreach}
				<input name="{$fldname}" tabindex="{$vt_tab}" id="jscal_field_{$fldname}" type="text" class="slds-input" size="11" maxlength="10"
				{*{if $OP_MODE == 'create' && ($fldname eq 'quotegeneration' || $fldname eq 'cf_919' || $fldname eq 'closedate')} {elseif $OP_MODE == 'create' && ($fldname eq 'cf_915' || $fldname eq 'closingdate')} value="{$today}" {else} value="{$date_val}" {/if}*} 
				...
				{if  $MODULE eq 'Potentials' && $fldname eq 'closingdate'} onchange="var t=this.value;var m = t.split('-');document.getElementById('annorif').value=m[2];document.getElementById('meserif').value=m[1];"{/if} >
					...
					{if $uitype eq 6}
						{if $fldname eq 'data_app'}
					{else}
						{if $MODULE eq 'Task'}
							<select name="time_start" class="slds-select" tabindex="{$vt_tab}" >{$opts}</select>
						{else}
							<input name="time_start" tabindex="{$vt_tab}" class="slds-input" size="5" maxlength="5" type="text" value="{$time_val}">
						{/if}
					{/if}
				{/if}
				...
				{if $uitype eq 23 && ($QCMODULE eq 'Event' || $MODULE eq 'Task')}
					<select name="time_end" tabindex="{$vt_tab}" class="slds-select">{$opte}</select>
				{/if}
		...
		{elseif $uitype eq 63}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input name="{$fldname}" type="text" class="slds-input" size="2" value="{$fldvalue}" tabindex="{$vt_tab}" >&nbsp;
				<select name="duration_minutes" tabindex="{$vt_tab}" class="slds-select">
		...
		{elseif $uitype eq 68 || $uitype eq 66 || $uitype eq 62}
			<td width="20%" class="dvtCellLabel" align=right>
				{if $fromlink eq 'qcreate'}
					<select class="slds-select" name="parent_type" onChange='document.QcEditView.parent_name.value=""; document.QcEditView.parent_id.value=""'>
				{else}
					<select class="slds-select" name="parent_type" onChange='document.EditView.parent_name.value=""; document.EditView.parent_id.value=""'>
				{/if}
		...
		{elseif $uitype eq 59}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input name="{$fldname}" type="hidden" value="{$secondvalue}">
				<input name="product_name" readonly type="text" class="slds-input" value="{$fldvalue}">&nbsp;
				<img tabindex="{$vt_tab}" src="{'btnL3Add.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='return window.open("index.php?module=Products&action=Popup&html=Popup_picker&form=HelpDeskEditView&popuptype=specific&fromlink={$fromlink}","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");' align="absmiddle" style='width:18px;cursor:pointer;'>&nbsp;<input type="image" src="{'clear_field.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}" LANGUAGE=javascript onClick="this.form.product_id.value=''; this.form.product_name.value=''; return false;" align="absmiddle" style='width:18px;cursor:pointer;'>
			</td>
		{elseif $uitype eq 55 || $uitype eq 255}
			<td width="20%" class="dvtCellLabel" align=right>
				{if $MASS_EDIT eq '1' && $fldvalue neq ''}
					{$APP.Salutation}
					<span class="slds-checkbox">
						<input type="checkbox" name="salutationtype_mass_edit_check" id="salutationtype_mass_edit_check">
		...
		{elseif $uitype eq 22}
			...
			<td width="30%" align=left class="dvtCellInfo">
				<textarea name="{$fldname}" class="slds-textarea" cols="30" tabindex="{$vt_tab}" rows="2">{$fldvalue}</textarea>
			</td>
		{elseif $uitype eq 14}
			<td width="20%" class="dvtCellLabel" align=right>
		...
		{elseif $uitype eq 104}<!-- Mandatory Email Fields -->
			...
			<td width=30% align=left class="dvtCellInfo"><input type="text" name="{$fldname}" id ="{$fldname}" value="{$fldvalue}" tabindex="{$vt_tab}" class=detailedViewTextBox ></td>
		...
		{elseif $uitype eq 101}<!-- for reportsto field USERS POPUP -->
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				&nbsp;<input title="{$APP.LBL_CHANGE_TITLE}" accessKey="C" type="button" class="slds-button slds-button--small slds-button--brand" value='{$APP.LBL_CHANGE}' name="btn1" onclick='return window.open("index.php?module=Users&action=Popup&html=Popup_picker&form=vtlibPopupView&form_submit=false&fromlink={$fromlink}&recordid={$ID}&forfield={$fldname}","test","width=640,height=603,resizable=0,scrollbars=0");'>
		...
		{elseif $uitype eq 28}
			<td width="20%" class="dvtCellLabel" align=right>
				...
				{if $MASS_EDIT eq '1'}
					<span class="slds-checkbox">
						<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check" disabled>
							<label class="slds-checkbox__label" for="{$fldname}_mass_edit_check">
								<span class="slds-checkbox--faux"></span>
							</label>
					</span>
		...
		{elseif $uitype eq 83} <!-- Handle the Tax in Inventory -->
			{foreach item=tax key=count from=$TAX_DETAILS}
				...
				<td class="dvtCellInfo" align="left" style="border:0px solid red;">
					<input type="text" class="slds-select" name="{$tax.taxname}" id="{$tax.taxname}" value="{$tax.percentage}" style="visibility:{$show_value};" onBlur="fntaxValidation('{$tax.taxname}')">
				</td>
			{/foreach}
		...
		{elseif $MODULE eq 'Task' && $header eq $MOD.LBLDESCRIPTIONINFORMATION}
			...
			<td class="dvtCellInfo" align="left" style="border:0px solid red;">
				...
				<select {$disabled_field} name="contactlist" size=5  style="height: 100px;width: 300px"  id="parentid" class="slds-select" multiple>{$CONTACTSNAME}</select>
				<input type="button" onclick="selectContact_Task(document.EditView,document.getElementsByName('linktoentity').item(0).value);" class="slds-button slds-button--brand slds-button--small" name="selectcnt" value="{$APP.LBL_SELECT_CONTACT_BUTTON_LABEL}">
				<input type='button' value='{$APP.LNK_DELETE}' class="slds-button--destructive slds-button slds-button--small" onclick='removeActContacts();'>
			</td>
		{elseif $MODULE eq 'Potentials'}
			...
			<td class="dvtCellInfo" align="left" style="border:0px solid red;">
				...
				<input name="deletecntlist" id="deletecntlist" type="hidden">
				<select {$disabled_field} name="contactlist" size=5  style="height: 100px;width: 300px"  id="parentid" class="slds-select" multiple>{$CONTACTSNAME}</select>
				<input type="button" onclick="selectContact_Pot(document.getElementsByName('related_to').item(0).value);" class="slds-button slds-button--brand slds-button--small" name="selectcnt" value="{$APP.LBL_SELECT_CONTACT_BUTTON_LABEL}">
				<input type='button' value='{$APP.LNK_DELETE}' class="slds-button slds-button--destructive slds-button--small" onclick='removeActContacts();'>
			</td>
		{/if}