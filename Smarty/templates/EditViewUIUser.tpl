...
{else}
		...
		{elseif $uitype eq 56 && ($fldname neq 'superadmincheck' || ($fldname eq 'superadmincheck' && $is_superadm_or_admin) )}
			...
			{else}
				...
				{else}
					<td width="30%" align=left class="dvtCellInfo">
						<input name="{$fldname}" tabindex="{$vt_tab}" type="checkbox" {if ( $PROD_MODE eq 'create' &&  $fldname|substr:0:3 neq 'cf_') ||( $fldname|substr:0:3 neq 'cf_' && $PRICE_BOOK_MODE eq 'create' ) || ($USER_MODE eq 'create' && $fldname neq 'superadmincheck')}checked{/if}>
					</td>
				{/if}
			{/if}
