										...
										<div class="slds-media__body">
											...
											{if $OP_MODE eq 'create_view'}
												{if $DUPLICATE neq 'true'}
													<p class="slds-text-heading--label slds-line-height--reset" style="opacity: 1;">{$APP.LBL_CREATING} {$SINGLE_MOD|@getTranslatedString:$MODULE}</span> <br>
												{else}
													<p class="slds-text-heading--label slds-line-height--reset" style="opacity: 1;">{$APP.LBL_DUPLICATING} "{$NAME}" </span> <br>
												{/if}
											{/if}
										</div>
									</div>
