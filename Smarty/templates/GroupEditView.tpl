	...
	<div align=center>
		...
		<input type="hidden" name="returnaction" value="{$RETURN_ACTION}">
			<tr>
				...
				{if $MODE eq 'edit'}
				<td class=heading2 valign=bottom><b><a href="index.php?module=UserSettings&action=index">{'LBL_USERSETTINGS'|@getTranslatedString}</a> > <a href="index.php?module=Settings&action=listgroups&parenttab=Settings">{$CMOD.LBL_GROUPS}</a> &gt; {$MOD.LBL_EDIT} &quot;{$GROUPNAME}&quot; </b></td>
				{else}	
				<td class=heading2 valign=bottom><b><a href="index.php?module=UserSettings&action=index">{'LBL_USERSETTINGS'|@getTranslatedString}</a> > <a href="index.php?module=Settings&action=listgroups&parenttab=Settings">{$CMOD.LBL_GROUPS}</a> &gt; {$CMOD.LBL_CREATE_NEW_GROUP}</b></td>
				{/if}
			</tr>
			<tr>
				{if $MODE eq 'edit'}
