		...
		<td align="right">
			...
			{if $HEADER eq 'Contacts' && $MODULE neq 'Campaigns' && $MODULE neq 'Accounts' && $MODULE neq 'Potentials' && $MODULE neq 'Products' && $MODULE neq 'Vendors'}
				...
				{elseif $MODULE neq 'Services'}
					...
					<input title="{$APP.LBL_ADD_NEW} {$APP.Contact}" accessyKey="F" class="slds-button slds-button--small slds-button_success" onclick="this.form.action.value='EditView';this.form.module.value='Contacts'" type="submit" name="button" value="{$APP.LBL_ADD_NEW} {$APP.Contact}"></td>
				{/if}
