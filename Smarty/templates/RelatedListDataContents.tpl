...
<table border=0 cellspacing=0 cellpadding=0 width=100% class="small rel_mod_data_paging" style="border-bottom:1px solid #999999;padding:5px; background-color: #eeefff;">
    <tr>
        ...
        <td align="right">
            {if isset($RELATEDLISTDATA.CUSTOM_BUTTON)}{$RELATEDLISTDATA.CUSTOM_BUTTON}{/if} {if $HEADER eq 'Contacts' && $MODULE neq 'Campaigns' && $MODULE neq 'Accounts' && $MODULE neq 'Potentials' && $MODULE neq 'Products' && $MODULE neq 'Vendors'} {if $MODULE eq 'Calendar'}
        ...
        {/if} {elseif $HEADER eq 'Users' && $MODULE eq 'Calendar'}
        <input title="Change" accessKey="" tabindex="2" type="button" class="slds-button slds-button--small slds-button--brand" value="{$APP.LBL_SELECT_USER_BUTTON_LABEL}" name="button" onclick='return window.open("index.php?module=Users&return_module=Calendar&return_action={$return_modname}&activity_mode=Events&action=Popup&popuptype=detailview&form=EditView&form_submit=true&select=enable&return_id={$ID}&recordid={$ID}","test","width=640,height=525,resizable=0,scrollbars=0")' ;> {/if}
        </td>
    </tr>
</table>
<table border=0 cellspacing=1 cellpadding=3 width=100% style="background-color:#eaeaea; display: none;" class="small rel_mod_data">
    ...
    {foreachelse}
    <tr style="height: 25px;" bgcolor="white" class="rel_mod_data_emptyrow">
...
<!-- Lighting Design -->
<table class="slds-table slds-table--bordered slds-table--col-bordered slds-table--fixed-layout" role="grid">
    <!-- Table Headers -->
    <thead>
