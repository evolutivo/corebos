...
function addConditionRow(groupIndex) {ldelim}
	...
	{if $SOURCE eq 'reports'}
		node3.innerHTML = '<div class='slds-form-element__control'><input name="fval'+columnIndex+'" id="fval'+columnIndex+'" class="slds-input repBox" placeholder='Enter the text' type="text" value=""> </div>'+
 ...
 <div style="overflow:auto;" id='where_filter_div' name='where_filter_div'>
  ...
  <div class="slds-card__header slds-grid">
    <header class="slds-media slds-media--center slds-has-flexi-truncate">
	  ...
	  <div class="slds-no-flex">
		  <button  class="slds-button slds-button--neutral" name="addwhereconditions"  onClick="return validateCV();">Generate</button>
	  </div>
  </div>
  <div class="slds-card__body">
    <table class="slds-table slds-table--bordered slds-no-row-hover slds-table--cell-buffer" cellpadding="5" cellspacing="0" width="100%">
     <tr>
...
function checkAdvancedFilter() {
    ...
    var url = "index.php?module=MVCreator&action=MVCreatorAjax&file=generateFilterSql";
     $.ajax({
            type: "POST",
            url: url,
            ...
            {   
                jQuery("#generatedConditions").html(msg); if (box) box.remove();
            },
            error: function()
            {
