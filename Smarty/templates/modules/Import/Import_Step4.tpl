				...
				<tr class="fieldIdentifier" id="fieldIdentifier{$_COUNTER}">
					...
					<td class="cellLabel">
						...
						<select name="mapped_fields" class="txtBox" style="width: 100%" onchange="ImportJs.loadDefaultValueWidget('fieldIdentifier{$_COUNTER}')">
							...
							<option value="{$_FIELD_NAME}" {if $_HEADER_NAME eq $_TRANSLATED_FIELD_LABEL || $_HEADER_NAME eq $_FIELD_NAME} selected {/if} >{$_TRANSLATED_FIELD_LABEL}{if $_FIELD_INFO->isMandatory() eq 'true'}&nbsp; (*){/if}</option>
							{/foreach}
						</select>
					</td>
