...
class CRMEntity {
	...
	function uploadAndSaveFile($id, $module, $file_details, $attachmentname='', $direct_import=false, $forfield='') {
		...
		if ($upload_status) {
			...
			if ((isset($_REQUEST['mode']) and $_REQUEST['mode'] == 'edit') or $this->mode == 'edit') {
...
// Code included by Jaguar - Ends
		...
		if ($cachedModuleFields === false) {
			...
			if ($noofrows) {
				while ($resultrow = $adb->fetch_array($result1)) {
					...
					VTCacheUtils::updateFieldInfo($tabid, $resultrow['fieldname'], $resultrow['fieldid'], $resultrow['fieldlabel'],
						$resultrow['columnname'], $resultrow['tablename'], $resultrow['uitype'], $resultrow['typeofdata'], $resultrow['presence']);
				}
			}
		...
		if ($actions) {
			...
			if (in_array('SELECT', $actions) && isPermitted($related_module, 4, '') == 'yes') {
				...
				if (!$racbr or $racbr->hasRelatedListPermissionTo('select',$related_module)) {
					$button .= "<input title='" . getTranslatedString('LBL_SELECT') . " " . getTranslatedString($related_module,$related_module) . "'class='slds-button slds-button--small slds-button--brand' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='" . getTranslatedString('LBL_SELECT') . " " . getTranslatedString($related_module,$related_module) . "'>&nbsp;";
				}
			}
			if (in_array('ADD', $actions) && isPermitted($related_module, 1, '') == 'yes') {
				...
				if (!$racbr or $racbr->hasRelatedListPermissionTo('create',$related_module)) {
					...
					$button .= "<input type='hidden' name='createmode' value='link' />" .
						"<input title='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "' class='slds-button slds-button--small slds-button_success'" .
	...
	function get_emails($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if ($actions) {
			...
			if (in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module,$related_module). "' class='slds-button slds-button--brand slds-button--small' aria-live='assertive' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module,$related_module) ."'>&nbsp;";
			}
			if (in_array('ADD', $actions) && isPermitted($related_module,1, '') == 'yes') {
				$singular_modname = getTranslatedString('SINGLE_' . $related_module,$related_module);
				$button .= "<input title='". getTranslatedString('LBL_ADD_NEW')." ". $singular_modname."' accessyKey='F' class='slds-button slds-button_success slds-button--small' onclick='fnvshobj(this,\"sendmail_cont\");sendmail(\"$this_module\",$id);' type='button' name='button' value='". getTranslatedString('LBL_ADD_NEW')." ". $singular_modname."'></td>";
			}
		}
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($actions) {
			...
			if (in_array('SELECT', $actions) && isPermitted($related_module, 4, '') == 'yes') {
				...
				$racbr = $wfs->getRACRuleForRecord($currentModule, $id);
				if (!$racbr or $racbr->hasRelatedListPermissionTo('select',$related_module)) {
					$button .= "<input title='" . getTranslatedString('LBL_SELECT') . " " . getTranslatedString($related_module, $related_module) . "' class='slds-button slds-button--brand slds-button--small' " .
			...
			if (in_array('ADD', $actions) && isPermitted($related_module, 1, '') == 'yes') {
				...
				if (!$racbr or $racbr->hasRelatedListPermissionTo('create',$related_module)) {
					$singular_modname = getTranslatedString('SINGLE_' . $related_module, $related_module);
					$button .= "<input type='hidden' name='createmode' value='link' />" .
						"<input title='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "' class='slds-button slds-button_success slds-button--small' " .
						" onclick='this.form.action.value=\"EditView\";this.form.module.value=\"$related_module\"' type='submit' name='button'" .
						" value='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "'>&nbsp;";
				}
			}
		...
		if ($numOfFields > 0) {
			...
			if ($actions) {
				...
				if (in_array('ADD', $actions) && isPermitted($related_module, 1, '') == 'yes'
					...
					if (!$racbr or $racbr->hasRelatedListPermissionTo('create',$related_module)) {
						$singular_modname = getTranslatedString('SINGLE_' . $related_module, $related_module);
						$button .= "<input title='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "' class='slds-button slds-button_success slds-button--small'" .
							" onclick='this.form.action.value=\"EditView\";this.form.module.value=\"$related_module\"' type='submit' name='button'" .
							" value='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "'>&nbsp;";
					}
				}
	...
	*/
		...
		if ($numOfFields > 0) {
			...
			if ($actions) {
				...
				if (in_array('ADD', $actions) && isPermitted($related_module, 1, '') == 'yes'
					...
					if (!$racbr or $racbr->hasRelatedListPermissionTo('create',$related_module)) {
						$singular_modname = getTranslatedString('SINGLE_' . $related_module, $related_module);
						$button .= "<input title='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "' class='slds-button slds-button--small slds-button_success'" .
							" onclick='this.form.action.value=\"EditView\";this.form.module.value=\"$related_module\"' type='submit' name='button'" .
							" value='" . getTranslatedString('LBL_ADD_NEW') . " " . $singular_modname . "'>&nbsp;";
					}
				}
	...
	function generateReportsQuery($module, $queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT vtiger_field.columnname,vtiger_field.tablename,vtiger_field.fieldid from vtiger_field INNER JOIN vtiger_tab on vtiger_tab.name = ? WHERE vtiger_tab.tabid=vtiger_field.tabid AND vtiger_field.uitype IN (10) and vtiger_field.presence in (0,2)", array($module));
			...
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					...
					if ($queryPlanner->requireTable($crmentityRelModuleFieldTable, $matrix)) {
						$relquery.= " left join vtiger_crmentity as $crmentityRelModuleFieldTable on $crmentityRelModuleFieldTable.crmid = $tab_name.$col_name and $crmentityRelModuleFieldTable.deleted=0";
					}
	...
	function generateReportsSecQuery($module, $secmodule,$queryPlanner,$type = '',$where_condition = '') {
		...
		$fields_query = $adb->pquery("SELECT vtiger_field.columnname,vtiger_field.tablename,vtiger_field.fieldid from vtiger_field INNER JOIN vtiger_tab on vtiger_tab.name = ? WHERE vtiger_tab.tabid=vtiger_field.tabid AND vtiger_field.uitype IN (10) and vtiger_field.presence in (0,2)", array($secmodule));
			...
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					...
					if ($queryPlanner->requireTable($crmentityRelSecModuleTable, $matrix)) {
						$relquery .= " left join vtiger_crmentity as $crmentityRelSecModuleTable on $crmentityRelSecModuleTable.crmid = $tab_name.$col_name and $crmentityRelSecModuleTable.deleted=0";
					}
					for ($j = 0; $j < $adb->num_rows($ui10_modules_query); $j++) {
						$rel_mod = $adb->query_result($ui10_modules_query, $j, 'relmodule');
						$rel_obj = CRMEntity::getInstance($rel_mod);
...
//                  $user = getUserName($row->_source->userchange);
		...
		$return_value = Array('header'=>$header,'entries'=>$entries);
		$log->debug("Exiting from get_log_historydenorm($entityid,$tabid method ...");
		return $return_value;
	}
       function get_log_historynorm($entityid,$tabid)
	{
		...
		$moduleName = getTabModuleName($tabid);
                ...
                foreach ($response1->hits->hits as $row) {
                 ...
                 $cr='modifiedtime'.$moduleName;
                 $update_date = $row->_source->$cr;
                 $lines = array();
               ...
               {   if($oldvl!='')                    
                ...
                utf8_encode(strftime('%c', strtotime($update_date))),$user);               
               }
		$return_value = Array('header'=>$header,'entries'=>$entries);
		$log->debug("Exiting from get_log_historynorm($entityid,$tabid method ...");
		return $return_value;
	}
       function get_log_historyoldlog($entityid,$tabid)
	{
		global $log, $adb,$current_user;
		
		$moduleName = getTabModuleName($tabid);
                $log->debug("Entering into get_log_historyoldlog($entityid,$tabid) method ...");
                include_once('modules/LoggingConf/LoggingUtils.php');
                $pref=GlobalVariable::getVariable('ip_elastic_indexprefix', '');
                $indextype=strtolower($pref.$moduleName);
                $tab=$adb->query("select * from vtiger_entityname where tabid=$tabid");
                $idfield=$adb->query_result($tab,0,'entityidfield');
                $mainfld=$idfield.$moduleName;
                $header=Array();
                $header[0] ="".getTranslatedString('LBL_ACTION');
                $header[1] ="".getTranslatedString('LBL_DATE');
                $header[2] ="".getTranslatedString('LBL_USER');
		...
		$return_value = Array('header'=>$header,'entries'=>$entries);
		$log->debug("Exiting from get_log_historyoldlog($entityid,$tabid method ...");
		return $return_value;
	}
}
?>
