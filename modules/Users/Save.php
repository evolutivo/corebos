...
if (isset($_POST['record']) && !is_admin($current_user) && !UserSettingsPermissions() && $_POST['record'] != $current_user->id) {
	echo ("Unauthorized access to user administration.");
} elseif (!isset($_POST['record']) && !is_admin($current_user) && !UserSettingsPermissions()) {
	echo ("Unauthorized access to user administration.");
}
...
if (empty($_REQUEST['changepassword']) || $_REQUEST['changepassword'] != 'true') {
	if (!UserSettingsPermissions() && strtolower($current_user->is_admin) == 'off' && $current_user->id != $focus->id) {
