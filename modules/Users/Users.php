...
class Users extends CRMEntity {
	...
	public function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if ($columname == 'is_admin' && !is_admin($current_user) and !UserSettingsPermissions()) {// only admin users can change admin field
	...
	public function save($module_name, $fileid = '') {
		...
		if (!UserSettingsPermissions() && !is_admin($current_user) && $current_user->id != $this->id) {// only admin users can change other users profile
			return false;
		}
