...
if (is_admin($current_user))
	$rsapps=$adb->pquery('select evvtappsid from vtiger_evvtapps
	 ...
	 where userid=? and evvtappsid!=1 and evvtappsid!=2 and evvtappsid!=3 group by appid order by installdate',array($current_user->id));
else {
	...
	$rsapps=$adb->pquery('select evvtappsid from vtiger_evvtapps
	 ...
	 where userid=? and wenabled and evvtappsid!=1 and evvtappsid!=2 and evvtappsid!=3 group by appid order by installdate',array($current_user->id));
}
...
for ($app=0;$app<$numapps;$app++) {
	...
	$windiv.="><img src='".$newApp->getAppIcon()."'><div id='appTitle'>".$newApp->getAppName($current_language)."</div></div>";
...
$numdel=$adb->getone("select count(*) from vtiger_evvtappsuser where appid=1 and wenabled=1 and userid=".$current_user->id);

if (is_admin($current_user) || $numdel>0) {
include_once "$mypath/vtapps/app1/vtapp.php";
$newApp=new vtAppcomTSolucioTrash(1);
?>
<div id='evvtapptrash' class='evvtappbox' title='<?php echo $newApp->getTooltipDescription($current_language); ?>'><img src='<?php echo $newApp->getAppIcon(); ?>'></div>
