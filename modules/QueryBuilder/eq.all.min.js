...
(function(d, c) {
    var e = function() {
        ...
        var i = 10900;
        var f;
        f = d.fn.jquery ? true : false;
        if (f) {
            var g = d.fn.jquery.split(".");
            var h = (10000 * parseInt(g[0])) + (100 * parseInt(g[1])) + parseInt(g[2] || 0);
            f = h >= j
        }
        if (!f) {
        ...
        f = d.ui.version ? true : false;
        if (f) {
            var g = d.ui.version.split(".");
            var h = (10000 * parseInt(g[0])) + (100 * parseInt(g[1])) + parseInt(g[2] || 0);
            f = h >= i
        }
        if (!f) {
    ...
    e();
    var b = c.EQ = c.EQ || {};
    var a = {Locale: "en", AltMenuAttribute: "Attribute", AltMenuConstantExpression: "Constant expression", ButtonApply: "Apply", 
                ButtonCancel: "Cancel", ButtonOK: "OK", ButtonClear: "Clear", ButtonEnable: "Toggle enable", ButtonDelete: "Delete", 
                ButtonAddCondition: "Add condition", ButtonAddPredicate: "Add group of conditions", 
                CmdAddConditionAfter: "Add a new condition after the current row", CmdAddConditionInto: "Add a new condition", 
                CmdAddPredicateAfter: "Open a bracket after the current row", CmdAddPredicateInto: "Open a bracket",
                CmdClickToAddCondition: "[Add new condition]", CmdDeleteRow: "Delete this row", ErrIncorrectPredicateTitleFormat: 
                "Incorrect predicate title format", ErrNotNumber: " is not a number", ErrIncorrectInteger: "Incorrect integer value", 
                ErrIncorrectNumberList: "Incorrect list format", False: "False", LinkTypeAll: "all", LinkTypeAny: "any", LinkTypeNone: 
                "none", LinkTypeNotAll: "not all", ConjAll: "and", ConjAny: "or", ConjNotAll: "and", ConjNone: "or", MsgApplySelection: 
                "[Apply selection]", MsgAs: "as", MsgEmptyList: "(empty list)", MsgEmptyListValue: "[select value]", MsgEmptyScalarValue:
                "[enter value]", MsgSubQueryValue: "[edit sub-query]", MsgEmptyAttrValue: "[select attribute]", MsgOf: "of", PredicateTitle: "{lt} of the following apply", RootPredicateTitle: "Select records where {lt} of the following apply", StrAddConditions: "Add conditions", SubQueryDialogTitle: "Edit sub-query", SubQueryColumnTitle: "Column:", SubQueryEmptyColumn: "[select column]", SubQueryQueryPanelCaption: "Conditions", True: "True", ButtonSorting: "Sorting", ButtonToAggr: "Change to aggregate column", ButtonToSimple: "Change to simple column", CmdAscending: "Ascending",
                CmdClickToAddColumn: "[Add new column]", CmdDeleteColumn: "Delete column", CmdDeleteSorting: "Delete sorting", CmdDescending: "Descending", CmdGroupSort: "Sorting", CmdNotSorted: "Not sorted", ColTypeAggrFunc: "Aggregate function", ColTypeCompound: "Calculated", ColTypeGroup: "Column type", ColTypeSimple: "Simple column", HeaderExpression: "Expression", HeaderSorting: "Sorting", HeaderTitle: "Title", SortHeaderColumn: "Column", SortHeaderSorting: "Sorting", StrAddColumns: "Add columns", CmdMoveToStart: "Move to start", CmdMoveRight: "Move right", CmdMoveLeft: "Move left", CmdMoveToEnd: "Move to the end", ButtonMenu: "Show menu", CmdToSimple: "Not aggregated", CmdMoveToFirst: "Move to the first", CmdMoveToPrev: "Move to the previous", CmdMoveToNext: "Move to the next", CmdMoveToLast: "Move to the last", StrNoFilterDefined: "No filter defined", StrNoFilterClickToAdd: "No filter defined. Click to add a new condition", Today: "Today", Yesterday: "Yesterday", Tomorrow: "Tomorrow", FirstDayOfMonth: "First day of the month", LastDayOfMonth: "Last day of the month", FirstDayOfWeek: "First day of the week", FirstDayOfYear: "First day of the year", FirstDayOfNextWeek: "First day of the next week", FirstDayOfNextMonth: "First day of the next month", FirstDayOfNextYear: "First day of the next week", Now: "Now", HourStart: "This hour start", Midnight: "Midnight", Noon: "Noon"};
    b.core = {texts: {Entities: {}, Attributes: {}, Operators: {}}, constLists: {SpecDateValues: [{id: "${Today}", key: "Today", isDefault: true}, 
                {id: "${Yesterday}", key: "Yesterday"}, {id: "${Tomorrow}", key: "Tomorrow"}, {id: "${FirstDayOfMonth}", key: "FirstDayOfMonth"},
                {id: "${FirstDayOfYear}", key: "FirstDayOfYear"}], SpecTimeValues: [{id: "${Now}", key: "Now", isDefault: true},
                {id: "${HourStart}", key: "HourStart"}, {id: "${Midnight}", key: "Midnight"}, {id: "${Noon}", key: "Noon"}], 
                    BooleanValues: [{id: "${false}", key: "False"}, {id: "${true}", key: "True", isDefault: true}]}, 
                predicateLinkTypeList: [{id: "All", key: "LinkTypeAll"},{id: "And", key: "LinkTypeAnd"},{id: "Or", key: "LinkTypeOr"}, {id: "Any", key: "LinkTypeAny"}, 
                {id: "None", key: "LinkTypeNone"}, {id: "NotAll", key: "LinkTypeNotAll"}], 
                _numericTypes: ["Byte", "Word", "Int", "Int32", "Int64", "Float", "Currency"], 
                _intTypes: ["Byte", "Word", "Int", "Int32", "Int64"], getText: function() {
            var j = b.core.texts;
            var h = "";
            if (arguments) {
                var g = false;
            ...
            return h
        }, getShiftToFitWindow: function(h, j) {
            var g = d(c).width();
            var i = h + j;
            var f = 0;
            if (i > g) {
            ...
            return f
        }, isNumericType: function(g) {
            var f = d.inArray(g, this._numericTypes);
            return(f >= 0)
        }, isIntType: function(g) {
            var f = d.inArray(g, this._intTypes);
            return(f >= 0)
        }, isNumeric: function(f) {
            return d.isNumeric(f)
        }, areCompatibleDataTypes: function(g, f) {
            return !g || !f || (g == f) || (g == "Date" && f == "DateTime") || (g == "DateTime" && f == "Date")
        }, combinePath: function(h, g) {
            var f = h;
            ...
            g.splice(f, 0, g.splice(i, 1)[0])
        }, queryChanged: function(g, f) {
            b.client.queryChanged(g, f)
        }, getEmptyQueryObject: function() {
            var f = {root: {linkType: "All", enabled: true, conditions: []}, columns: [], justsorted: []};
            return f
        }, parseOperatorFormat: function(g) {
            var h = b.core.getText("Operators", g.id, "displayFormat");
            if (!h) {
                h = g.displayFormat
            }
            var f = [];
            var i = new b.core.FormatParser(h);
            while (i.next()) {
                if (i.token === "operator") {
                    f.push({type: "operator", text: i.tokenText})
                } else {
                    if (i.token === "expression") {
                        f.push({type: "expression", index: i.exprNum - 1})
                    } else {
                        if (i.token === "text") {
                            f.push({type: "text", text: i.tokenText})
                        }
                    }
                }
            }
            return f
        }, isPropSet: function(g, f) {
            return g[f] || g[f.toLowerCase()] || g[f.toUpperCase()]
        }};
    b.core.FormatParser = function(f) {
        this.start(f)
    };
    b.core.FormatParser.prototype = {start: function(f) {
            this.formatStr = f;
            this.pos = 0;
            this.exprNum = 0;
            this.tokenText = ""
            ...
            if (this.formatStr.charAt(this.pos) === "{") {
                h = this.formatStr.indexOf("}", this.pos);
                if (h < 0) {
                    return false
                }
                this.tokenText = this.formatStr.substring(this.pos, h + 1);
                ...
                if (this.formatStr.charAt(this.pos) === "[" && this.pos < this.formatStr.length - 1 && this.formatStr.charAt(this.pos + 1) === "[") {
                    ...
                    h = this.formatStr.indexOf("]]", this.pos);
                    this.token = "operator";
                    this.tokenText = this.formatStr.substring(this.pos, h);
                    this.pos = h + 2
                } else {
                    this.token = "text";
                    var g = this.formatStr.indexOf("{", this.pos);
                    if (g < 0) {
                        g = this.formatStr.length
                    }
                    ...
                    h = Math.min(g, f);
                    this.tokenText = d.trim(this.formatStr.substring(this.pos, h));
                    this.pos = h
                }
            }
            return true
        }};
    b.core.DataModel = function(f) {
        if (f) {
            this.setObject(f)
        } else {
            this.setObject({aggrFunctions: [], operators: [], rootEntity: {}})
        }
    };
    b.core.DataModel.prototype = {nullAttribute: {id: "", caption: "{Unrecognized attribute}", dataType: "String", description: "", size: 0, operators: [], uic: false, uir: false, uis: false}, nullOperator: {id: "", caption: "{Unrecognized operator}", displayFormat: 
                    "{expr1} [[{unrecognized operator}]] {expr2}", exprType: "Unknown", paramCount: 2, valueKind: "Scalar"}, dateMacroList: ["${Today}", "${Yesterday}", "${Tomorrow}", "${FirstDayOfMonth}", "${LastDayOfMonth}", "${FirstDayOfWeek}", "${FirstDayOfYear}", "${FirstDayOfNextWeek}", "${FirstDayOfNextMonth}", "${FirstDayOfNextYear}"], timeMacroList: ["${Now}", "${HourStart}", "${Midnight}", "${Noon}"], getObject: function() {
            return this.model
            ...
            if (f) {
                ...
                this.runThroughEntities(function(g) {
                    if (g.UIC != undefined && g.uic == undefined) {
                        g.uic = g.UIC
                    }
                    if (g.UIR != undefined && g.uir == undefined) {
                        g.uir = g.UIR
                    }
                    if (g.UIS != undefined && g.uis == undefined) {
                        g.uis = g.UIS
                    }
                }, function(g) {
                    if (g.UIC != undefined && g.uic == undefined) {
        ...
        }, getAttributeById: function(g) {
            var f = this.getEntityAttrById(this.getRootEntity(), g);
            if (!f) {
                f = this.nullAttribute
            }
            return f
        }, checkAttrProperty: function(h, g) {
            var f = (typeof h === "object") ? h : this.getAttributeById(h);
            if (f) {
                if (f[g]) {
                    return true
                } else {
        ...
        }, getEntityAttrById: function(g, i) {
            var f;
            if (g.attributes) {
                var h = g.attributes.length;
                for (f = 0; f < h; f++) {
                    if (g.attributes[f].id == i) {
                        return g.attributes[f]
                    }
                }
            }
            if (g.subEntities) {
                var j = g.subEntities.length;
                for (f = 0; f < j; f++) {
                    res = this.getEntityAttrById(g.subEntities[f], i);
                    if (res) {
                        return res
                    }
                }
            ...
            return null
        }, getFullEntityPathByAttr: function(g, f) {
            f = f || " ";
            return this.getEntityPathByAttr(this.getRootEntity(), g, f)
        }, getEntityPathByAttr: function(g, i, o) {
            if (!g) {
                return""
            }
            o = o || " ";
            var m = "";
            if (g.caption) {
                var j = b.core.getText("Entities", g.caption);
                m = j ? j : g.caption
            }
            var n;
            if (g.attributes) {
                var f = g.attributes.length;
                for (n = 0; n < f; n++) {
                    if (g.attributes[n].id == i) {
                        return m
                    }
                }
            }
            if (g.subEntities) {
                var h = g.subEntities.length;
                for (n = 0; n < h; n++) {
                    var l = g.subEntities[n];
                    var k = this.getEntityPathByAttr(l, i, o);
                    if (k !== "") {
                        if (m !== "") {
                            k = m + o + k
                        }
            ...
            return""
        }, getAttributeText: function(g, h) {
            var j = b.core.getText("Attributes", g.id);
            if (!j) {
                j = g.caption
            }
            if (!h) {
            ...
            var f = h.replace(new RegExp("{attr}", "g"), j);
            var i = this.getFullEntityPathByAttr(g.id, ".");
            f = f.replace(new RegExp("{entity}", "g"), i);
            return f
        }, _listByEntityWithFilter: function(i, n) {
            var q = [];
            var o;
            var m = null;
            if (i.subEntities) {
                ...
                for (var f = 0; f < j; f++) {
                    m = i.subEntities[f];
                    if (!n || n(m)) {
                        o = b.core.getText("Entities", m.name);
                        if (!o) {
                            o = m.caption
                        }
                        var l = {id: m.name, text: o, items: []};
                        l.items = this._listByEntityWithFilter(m, n);
                        if (l.items.length > 0) {
                            q.push(l)
                        }
                    }
            ...
            if (i.attributes) {
                ...
                for (var g = 0; g < h; g++) {
                    k = i.attributes[g];
                    if (!n || n(k)) {
                        o = b.core.getText("Attributes", k.id);
                        if (!o) {
                            o = k.caption
                        }
                        q.push({id: k.id, text: o, dataType: k.dataType})
                    }
                }
            }
            return q
        }, _listByEntity: function(j, f) {
            var f = f || {};
            var q = [];
            var o;
            var n = null;
            if (j.subEntities) {
                var k = j.subEntities.length;
                for (var g = 0; g < k; g++) {
                    n = j.subEntities[g];
                    if (f.addUIC !== false && n.uic !== false || f.addUIR !== false && n.uir !== false || f.addUIS !== false && n.uis !== false) {
                        o = b.core.getText("Entities", n.name);
                        if (!o) {
                            o = n.caption
                        }
                        var m = {id: n.name, text: o, items: []};
                        m.items = this._listByEntity(n, f);
                        q.push(m)
                    }
                }
            }
            var l = null;
            if (j.attributes) {
                var i = j.attributes.length;
                for (var h = 0; h < i; h++) {
                    l = j.attributes[h];
                    if (f.addUIC !== false && l.uic !== false || f.addUIR !== false && l.uir !== false || f.addUIS !== false && l.uis !== false) {
                        o = b.core.getText("Attributes", l.id);
                        if (!o) {
                            o = l.caption
                        }
                        q.push({id: l.id, text: o, dataType: l.dataType})
                    }
                }
            }
            return q
        }, getEntitiesTree: function(f) {
            return this._listByEntity(this.getRootEntity(), f)
        }, getEntitiesTreeWithFilter: function(f) {
            return this._listByEntityWithFilter(this.getRootEntity(), f)
        }, _findItemById: function(i, h) {
            var g = i.length;
            for (var f = 0; f < g; f++) {
                if (i[f].id === h) {
                    return i[f]
        ...
        }, getFirstUICAttrInEntity: function(h) {
            if (h.uic !== false) {
                var g = 0;
                if (h.attributes) {
                    var i = h.attributes.length;
                    for (g = 0; g < i; g++) {
                        if (h.attributes[g].uic) {
                            return h.attributes[g]
                        }
                    }
                }
                if (h.subEntities) {
                    var j = h.subEntities.length;
                    for (g = 0; g < j; g++) {
                        var f = this.getFirstUICAttrInEntity(h.subEntities[g]);
                        if (f) {
                            return f
                        }
                    }
            ...
            return null
        }, runThroughEntities: function(f, g) {
            var h = {stop: false};
            var i = function(l) {
                if (g) {
                    g(l, h)
                ...
                var k = 0;
                if (l.attributes) {
                    var m = l.attributes.length;
                    for (k = 0; (k < m) && !h.stop; k++) {
                        var j = l.attributes[k];
                        f(j, h);
                        if (h.stop) {
                            return
                        }
                ...
                if (l.subEntities) {
                    var n = l.subEntities.length;
                    for (k = 0; (k < n) && !h.stop; k++) {
                        i(l.subEntities[k])
                    }
                }
            };
            i(this.getRootEntity())
        }, getFirstAttributeByFilter: function(g) {
            var f = null;
            this.runThroughEntities(function(h, i) {
                if (g(h)) {
                    i.stop = true;
                    f = h
                }
            ...
            return f
        }, findOperatorById: function(g) {
            if (this.model.operators) {
                var h = this.model.operators.length;
                for (var f = 0; f < h; f++) {
                    if (this.model.operators[f].id == g) {
                        return this.model.operators[f]
                    }
                }
            }
            return null
        }, getOperatorById: function(f) {
            var g = this.findOperatorById(f);
            if (!g) {
                g = this.nullOperator
            }
            return g
        }, getDefaultOperatorIdForAttr: function(f) {
            if (f.defaultOperator) {
                return f.defaultOperator
            } else {
                if (f.operators.length > 0) {
                    return f.operators[0]
                } else {
                    return this.nullOperator.id
                }
            }
        }, getDefaultOperatorForAttr: function(f) {
            operatorId = this.getDefaultOperatorIdForAttr(f);
            return this.getOperatorById(operatorId)
        }, getOperand: function(g, h, i) {
            var f = {kind: h && h.defaultOperand ? h.defaultOperand.kind : "Scalar", dataType: h && h.defaultOperand ? h.defaultOperand.dataType : "Unknown", defValue: h && h.defaultOperand && h.defaultOperand.defValue ? h.defaultOperand.defValue : "", editor: h && h.defaultOperand && h.defaultOperand.editor ? d.extend({}, h.defaultOperand.editor) : {type: "Unknown"}};
            if ((!f.dataType || f.dataType === "Unknown") && g) {
                f.dataType = g.dataType
            }
            if (h && (i !== undefined)) {
                if (h.operands && h.operands[i - 1]) {
                    d.extend(f, h.operands[i - 1])
                }
            }
            if (!f.editor.type || f.editor.type == "Unknown") {
                if (g && g.defaultEditor) {
                    f.editor = d.extend({}, g.defaultEditor)
                }
            }
            if (!f.editor.type || f.editor.type == "Unknown") {
                if (f.dataType) {
        ...
        }, getAggrFunctionCaption: function(h) {
            var f = b.core.getText("Aggr" + h.replace(" ", "") + "_Caption");
            if (f) {
                return f
            }
            var g = this._findItemById(this.model.aggrFunctions, h);
            if (!g || !g.caption) {
                return h
            }
            return g.caption
        }, getAggrFunctionFormat: function(h) {
            var g = b.core.getText("Aggr" + h.replace(" ", "") + "_Format");
            if (g) {
                return g
            }
            var f = this._findItemById(this.model.aggrFunctions, h);
            if (!f || !f.displayFormat) {
                return""
            }
            return f.displayFormat
        }, getMacroDateValue: function(g) {
            var h = new Date();
            if (d.inArray(g, this.dateMacroList) >= 0) {
                switch (g) {
                    case"${Today}":
                        break;
                    case"${Yesterday}":
                    ...
                    case"${FirstDayOfNextYear}":
                        h.setFullYear(h.getFullYear() + 1, 0, 1);
                        break
                }
                return h
            }
            return null
        }, getMacroTimeValue: function(f) {
            var g = new Date();
            if (d.inArray(f, this.timeMacroList) >= 0) {
                switch (f) {
                    case"${Now}":
                        break;
                    case"${HourStart}":
    ...
    b.core.Query = function(g, h, f) {
        if (g) {
            this.model = g
        } else {
            this.model = new b.core.DataModel()
        }
        h = h || b.core.getEmptyQueryObject();
        this._listCache = {};
        this._updating = 0;
        f = f || {};
        this.attrClassName = f.attrClassName || "ENTATTR";
        this.clientListRequestHandler = f.clientListRequestHandler;
        this.serverListRequestHandler = f.serverListRequestHandler;
        this._queryChangedCallbacks = d.Callbacks("unique");
        this.setObject(h)
    };
    b.core.Query.prototype = {getObject: function() {
            return this.query
        }, setObject: function(g, f) {
            if (g) {
                if (typeof g === "string") {
                    this.query = JSON.parse(g)
                } else {
                    this.query = g
                }
                if (!f) {
                    this.fireChangedEvent({changeType: "query"})
                }
            }
        }, getModel: function() {
            return this.model
            ...
            this.model = f;
            this.clear()
        }, loadModelObject: function(f) {
            this.model.setObject(f)
        }, toJSON: function() {
            var f = this.getModel();
            this.query.modelId = f ? f.getId() : null;
            this.query.modelName = f ? f.getName() : null;
            ...
            try {
                ...
                this.endUpdate();
                if (!f) {
                    this.fireChangedEvent({changeType: "clear"})
                }
            }
        }, clearColumns: function() {
            this.query.columns = [];
            this.query.justsorted = [];
            this.fireChangedEvent({changeType: "columns.clear"})
        }, clearConditions: function() {
            this.query.root = {linkType: "All", enabled: true, conditions: []};
            this.fireChangedEvent({changeType: "conditions.clear"})
        }, getId: function() {
        ...
        }, addColumn: function(h, f) {
            if (!this.query) {
                return
            }
            var g = this.query.columns;
            if (typeof f == "number") {
                g.splice.apply(g, [f, 0].concat(h))
            } else {
                g.push.apply(g, [].concat(h))
            }
            this.fireChangedEvent({changeType: "column.add", column: h})
        }, moveColumn: function(i, h) {
            var f = this.getColumns();
            var g = f[i];
            b.core.moveArrayItem(f, i, h);
            this.fireChangedEvent({changeType: "column.move", column: g})
        }, removeColumn: function(h) {
            var g = this.getColumns();
            var f = d.inArray(h, g);
            if (f >= 0) {
                g.splice(f, 1);
                this.fireChangedEvent({changeType: "column.remove", column: h})
            }
        }, getRootPredicate: function() {
            return this.query.root
        }, addSimpleCondition: function(l) {
            var g = l.predicate || this.getRootPredicate();
            var i = this.getModel();
            if (!i) {
                return null
            }
            var k = l.attr;
            var f = i.getAttributeById(k);
            if (!f) {
                return null
            }
            var h = l.operator;
            var m;
            if (!h) {
                m = i.getDefaultOperatorForAttr(f)
            } else {
                m = i.getOperatorById(h)
            }
            if (!m) {
                return null
            }
            var j = this._createSimpleConditionObject(f, m, l.value);
            g.conditions.push(j);
            this.fireChangedEvent({changeType: "condition.add", condition: j});
            return j
        }, _createValueExpr: function(g, h, j, k) {
            var i = this.getModel();
            var l = i.getOperand(g, h);
            var f = {typeName: "CONST", dataType: l.dataType, kind: l.kind, value: k, text: k};
            return f
        }, _createSimpleConditionObject: function(f, h, m) {
            var g = this;
            if (!f) {
                return null
            }
            ...
            var k = {typeName: "SMPL", enabled: true, operatorID: h.id, expressions: [{typeName: g.attrClassName, id: f.id}]};
            if (m) {
                if (d.isArray(m)) {
                    var l = m.length;
                    for (var j = 0; j < l; j++) {
                        k.expressions.push(this._createValueExpr(f, h, j + 1, m))
                    }
                } else {
                    k.expressions.push(this._createValueExpr(f, h, 1, m))
                }
            }
            return k
        }, _getListFromCache: function(g) {
            var f = g.listName == "SQL" ? "SQL_" + g.sql : g.listName;
            return this._listCache[f]
        }, _addListToCache: function(h, g) {
            var f = h.listName == "SQL" ? "SQL_" + h.sql : h.listName;
            this._listCache[f] = g
            ...
            var f = this;
            var g = function(l, i) {
                if (l == null || !l.listName) {
                    return
                }
                var h = l.listName.match(/{{(.+?)}}/);
                if (h) {
                    var k = f.getOneValueForAttr(h[1]);
                    if (!k) {
                        k = ""
                    }
                    l.listName = l.listName.replace(/{{.+?}}/, k)
                }
                var j = f._getListFromCache(l);
                if (j && j.length > 0) {
                    i(j);
                    return
                }
                var m = false;
                if (f.clientListRequestHandler) {
                    m = f.clientListRequestHandler(l, i)
                }
                if (!m && f.serverListRequestHandler) {
                    m = f.serverListRequestHandler(l, function(n) {
                        if (n) {
                            f._addListToCache(l, n)
                        }
                        i(n)
                    })
                }
            ...
            return g
        }, addChangedCallback: function(f) {
            if (!f) {
                return
            }
            this._queryChangedCallbacks.add(f)
        }, removeChangedCallback: function(f) {
            if (!f) {
                return
            }
            this._queryChangedCallbacks.remove(f)
        }, fireChangedEvent: function(f) {
            if (!this.isUpdating()) {
                f = f || {};
                f.query = this;
                this._queryChangedCallbacks.fire(f)
            }
        }, runThroughConditions: function(f) {
            var g = function(h) {
                var j;
                for (j = 0; j < h.conditions.length; j++) {
                    var k = h.conditions[j];
                    if (k.conditions) {
                        g(k)
                    } else {
                        if (f) {
            ...
            g(this.getRootPredicate())
        }, getOneValueForAttr: function(g) {
            var f = null;
            this.runThroughConditions(function(h) {
                var i = h.expressions[0];
                if (i.id == g && h.expressions[1] && (h.operatorID == "Equal" || h.operatorID == "InList" || h.operatorID == "StartsWith")) {
                    f = h.expressions[1].value
                }
            });
            return f
            ...
            var g = this.getModel();
            var f = function(x) {
                var y = "", r = "Conj" + x.linkType, k = b.core.getText(r), v, o;
                for (o = 0; o < x.conditions.length; o++) {
                    v = "";
                    var u = x.conditions[o];
                    if (u.enabled || typeof(u.enabled) === "undefined") {
                        if (u.conditions) {
                            v = f(u);
                            if (v) {
                                v = "(" + v + ")"
                            ...
                            var t = u.operatorID;
                            var j = g.getOperatorById(t);
                            var m = b.core.parseOperatorFormat(j);
                            var l = m.length;
                            for (var q = 0; q < l; q++) {
                                var h = m[q];
                                var n = h.text;
                                if (h.type == "expression") {
                                    var w = u.expressions[h.index];
                                    if (w.kind == "Attribute" || w.typeName == "ENTATTR") {
                                        var s = g.getAttributeById(w.id);
                                        n = g.getAttributeText(s, "{entity} {attr}")
                                    } else {
                                        n = w.value
                                    }
                                }
                ...
                if (x.linkType == "None" || x.linkType == "NotAll") {
                    y = "not ( " + y + " )"
                }
                return y
            };
            return f(this.getRootPredicate())
        }}
})(jQuery, window);
(function(f, g) {
    function e() {
        return{top: f(window).scrollTop(), left: f(window).scrollLeft()}
    }
    function h(l, m) {
        var k = {x: 0, y: 0};
        if (l !== null) {
            ...
            var i = f(m).offset();
            k = {x: j.left - i.left, y: j.top - i.top}
        }
        return k
    }
    function d() {
    ...
    var b = null;
    a.prototype = {_turnCheckboxes: function(j) {
            var k = j.length;
            var m;
            for (var l = 0; l < k; l++) {
                m = j[l];
                if (m.items) {
                    ...
                    if (m.itemCheckbox) {
                        m.itemCheckbox.checked = m.selected
                    }
                }
            }
            ...
            if (!m.subLevel) {
                m.subLevel = new a({menu: this.parentMenu, items: m.items, levelIndex: this.levelIndex + 1})
            }
            ...
            var w = this;
            var u = this.parentMenu.style.colors.bgON || "white";
            var n = this.parentMenu.style.colors.fgON || "black";
            var i = this.parentMenu.style.colors.bgOVER || "LightSteelBlue";
            var z = this.parentMenu.style.itemStyle.fontFamily || "";
            var l = this.parentMenu.style.itemStyle.fontSize || "14px";
            var v = this.parentMenu.options.multiselect;
            var q = document.createElement("div");
            if (this.parentMenu.options.useDefaultStyles) {
                q.style.backgroundColor = u;
                q.style.border = "1px solid";
                q.style.borderColor = this.parentMenu.style.colors.border;
                q.style.margin = "-2px 2px 2px -2px";
                q.style.width = "auto";
                q.style.height = "auto"
            }
            q.style.zIndex = this.parentMenu.zIndex;
            q.style.position = "absolute";
            q.style.display = "none";
            q.menuLevel = this;
            var k = this.parentMenu;
            var m = this.applyItem;
            if (v && this.levelIndex === 0) {
                var s = document.createElement("div"), o = document.createElement("button");
                f(s).addClass("eqjs-menu-applyDiv");
                s.menuItem = m;
                if (this.parentMenu.options.useDefaultStyles) {
                    s.style.borderBottom = "1px solid";
                    s.style.padding = "5px";
                    s.style.marginBottom = "5px";
                    o.style.padding = "0 5px";
                    o.style.cursor = "pointer"
                }
                var A = document.createTextNode(this.parentMenu.options.buttons.submit);
                o.appendChild(A);
                s.appendChild(o);
                var x = document.createElement("button");
                f(x).addClass("eqjs-menu-cancel");
                if (this.parentMenu.options.useDefaultStyles) {
                    x.style.padding = "0 5px";
                    x.style.cursor = "pointer";
                    x.style.marginLeft = "15px"
                }
                var t = document.createTextNode(this.parentMenu.options.buttons.cancel);
                x.appendChild(t);
                s.appendChild(x);
                q.appendChild(s);
                m.itemDiv = s;
                f(o).click(function() {
                    w.submit(w.applyItem)
                });
                f(x).click(function() {
                    w.parentMenu.hideMenu()
                })
            }
            if (w.items.length >= k.options.showSearchBoxAfter) {
                var r = document.createElement("div");
                f(r).addClass("eqjs-menu-searchDiv");
                if (this.parentMenu.options.useDefaultStyles) {
                    r.style.borderBottom = "1px solid #666";
                    r.style.backgroundColor = u;
                    r.style.borderColor = this.parentMenu.style.colors.border;
                    if (z != "") {
                        r.style.fontFamily = z
                    }
                    r.style.fontSize = l;
                    r.style.color = n;
                ...
                if (this.parentMenu.options.useDefaultStyles) {
                    ...
                    y.style.fontSize = "8pt";
                    y.style.width = "100%"
                }
                r.appendChild(y);
                q.appendChild(r);
                f(y).on("input", function() {
                    w._renderItems(f(this).val())
                })
            }
            var j = document.createElement("div");
            f(j).addClass("eqjs-menu-scrollDiv");
            ...
            for (A = 0; A < C; A++) {
                ...
                if (D.text == "---") {
                    ...
                    if (t && !D.items) {
                        ...
                        y.type = "checkbox";
                        y.id = "cb" + D.id;
                        y.checked = D.selected;
                        y.defaultChecked = D.selected;
                        q.appendChild(y);
                        D.itemCheckbox = y;
                        if (this.parentMenu.options.useDefaultStyles) {
                    ...
                    var s = function(F) {
                        ...
                        if (t) {
                            if (!i.items) {
                                ...
                                i.itemCheckbox.checked = i.selected
                            }
                        } else {
                            x.activateItem(i);
                            x.submit(i)
    ...
    function a(i) {
        ...
        this.parentElement = i.container || document.documentElement;
    ...
    var c = 1;
    f.widget("eqjs.PopupMenu", {_mouseIsOverBlock: false, _mouseIsOverLink: false, _toId: null, _itemSelectedCallback: null,
        _menuClosedCallback: null, _itemFilterCallback: null, _rootLevel: null, 
        options: {items: [], buttons: {submit: EQ.core.getText("ButtonApply"), cancel: EQ.core.getText("ButtonCancel")}, 
            itemFilterCallback: null, useDefaultStyles: false, multiselect: false, adjustHeight: true, showSearchBoxAfter: 30,
            activateOnMouseOver: true, container: document.body, zIndex: 100000}, _create: function() {
            this.menuId = c++;
            this._updateProps();
            this._rootLevel = new a({menu: this, items: this.options.items, levelIndex: 0, container: this.options.container});
            ...
            var j = {widget: this, menuItem: i, selectedItems: k};
...
(function(a) {
    ...
    $.extend(Timepicker.prototype, {$input: null, $altInput: null, $timeObj: null, inst: null, hour_slider: null, minute_slider: null, second_slider: null, millisec_slider: null, microsec_slider: null, timezone_select: null, maxTime: null, minTime: null, hour: 0, minute: 0, second: 0, millisec: 0, microsec: 0, timezone: null, hourMinOriginal: null, minuteMinOriginal: null, secondMinOriginal: null, millisecMinOriginal: null, microsecMinOriginal: null, hourMaxOriginal: null, minuteMaxOriginal: null, secondMaxOriginal: null, millisecMaxOriginal: null, microsecMaxOriginal: null, ampm: "", formattedDate: "", formattedTime: "", formattedDateTime: "", timezoneList: null, units: ["hour", "minute", "second", "millisec", "microsec"], support: {}, control: null, setDefaults: function(settings) {
    ...
    $.datepicker.parseTime = function(timeFormat, timeString, options) {
        ...
        var strictParse = function(f, s, o) {
            ...
            var getFormatPositions = function(timeFormat) {
                var finds = timeFormat.toLowerCase().match(/(h{1,2}|m{1,2}|s{1,2}|l{1}|c{1}|t{1,2}|z|'.*?')/g), orders = {h: -1, m: -1, s: -1, l: -1, c: -1, t: -1, z: -1};
                if (finds) {
            ...
            var resTime = {hour: 0, minute: 0, second: 0, millisec: 0, microsec: 0};
            if (treg) {
                if (order.t !== -1) {
    ...
    $.datepicker.formatTime = function(format, time, options) {
        ...
        time = $.extend({hour: 0, minute: 0, second: 0, millisec: 0, microsec: 0, timezone: null}, time);
    ...
    var splitDateTime = function(dateTimeString, timeSettings) {
        ...
        if (allPartsLen > 1) {
            return{dateString: allParts.splice(0, allPartsLen - timePartsLen).join(separator), timeString: allParts.splice(0, timePartsLen).join(separator)}
        }
    ...
    $.timepicker.handleRange = function(method, startTime, endTime, options) {
        options = $.extend({}, {minInterval: 0, maxInterval: 0, start: {}, end: {}}, options);
        var timeOnly = false;
        ...
        $.fn[method].call(startTime, $.extend({timeOnly: timeOnly, onClose: function(dateText, inst) {
                checkDates($(this), endTime)
            }, onSelect: function(selectedDateTime) {
                selected($(this), endTime, "minDate")
            }}, options, options.start));
        $.fn[method].call(endTime, $.extend({timeOnly: timeOnly, onClose: function(dateText, inst) {
    ...
    $.timepicker._util = {_extendRemove: extendRemove, _isEmptyObject: isEmptyObject, _convert24to12: convert24to12, _detectSupport: detectSupport, _selectLocalTimezone: selectLocalTimezone, _computeEffectiveSetting: computeEffectiveSetting, _splitDateTime: splitDateTime, _parseDateTimeInternal: parseDateTimeInternal};
...
(function(c, b) {
    var a = b.EQ = b.EQ || {};
    a.client = {defaultQuery: new a.core.Query(), serviceUrl: "EasyQuery", locale: "", _currentOptions: {}, _modelPromise: null,
        _updateActionUrls: function() {
            ...
            l.queryPanel = l.queryPanel || {};
                ...
                c.ajax({type: "POST", url: j.listRequestUrl, data: m, dataType: "json", success: function(q) {
                        n(q)
            ...
            if (this.modelId || l.loadDefaultModel) {
                ...
                this.loadModel({modelId: i, modelName: i, success: function(n) {
                        var m = j.getQuery();
            ...
            var d = this;
            var e = this._addAntiForgeryToken({modelId: f.modelId, modelName: f.modelName});
            if (d.controls.EPWidget) {
                d.controls.EPWidget.EntitiesPanel("startLoading")
            }
           var selected_elements=[];

           $.each($('.div.eqjs-qc-colelement,.eqjs-qc-captionelement'),function(key,element){
               selected_elements.push(element.innerText);
           });

           this._modelPromise = c.ajax({type: "POST", url: a.client.loadModelUrl, data: e+'&selected_elements='+JSON.stringify(selected_elements), dataType: "json", beforeSend: f.beforeSend, crossDomain: false, success: function(g) {
            ...
            var d = this;
            var e = this._addAntiForgeryToken({queryName: f.queryName, modelId: f.modelId});
            d._doWhenModelLoaded(function() {
                c.ajax({type: "POST", url: a.client.newQueryUrl, data: e, dataType: "json", beforeSend: f.beforeSend, success: function(g) {
            ...
            var d = this;
            var e = this._addAntiForgeryToken({queryId: f.queryId, queryName: f.queryName, modelId: f.modelId});
            d._doWhenModelLoaded(function() {
                c.ajax({type: "POST", url: a.client.loadQueryUrl, data: e, //dataType: "json",
                    beforeSend: f.beforeSend, success: function(g) {

                        var maps='<table style="margin-top:20px " ><tr>\n\
                                        <th style=" width:44%;padding: 8px 3px 0 0;"> Map Name </th>\n\
                                        <th style=" width:33%;padding: 8px 3px 0 0;"> description</th>\n\
                                        <th style=" width:13%;padding: 8px 3px 0 0;"> Select Map </th>\n\
                                   </tr>';

                        var mapinfo=jQuery.parseJSON(g);
                        for(var mapid in mapinfo){
                            maps+='<tr><td  style=" width:44%;padding: 8px 0 0 0;">'+mapinfo[mapid]['MapName']+'</td>\n\
                                       <td style=" width:33%;padding: 8px 0 0 0;"> <input type="hidden" id="map'+mapid+'" value=\''+(JSON.stringify(mapinfo[mapid]['queryJson']))+'\' /> '+(JSON.stringify(mapinfo[mapid]['queryJson'])).substring(70,220)+'</td>\n\
                                       <td style=" width:13%;padding: 8px 0 0 0;"> <center><input type="checkbox" id="check'+mapid+'" onclick="changeQuery( '+mapid +'  )" /></center>\n\
                                       <input type="hidden" id="mapOp'+mapid+'" value=\''+(JSON.stringify(mapinfo[mapid]['selectedOperators']))+'\'  /></td></tr>';
                        }

                        maps+='</table>' ;
                        
                        $("#dialog-message").append(maps)
            ...
            var d = this;
            var g = f.modelId;
            var e = this._addAntiForgeryToken({modelId: g});
            c.ajax({type: "GET", url: this.loadQueryListUrl, data: e, dataType: "json", beforeSend: f.beforeSend, success: function(h) {
                    if (f.success) {
                        f.success(h)
                    }
            ...
            var d = this._addAntiForgeryToken({queryId: e.queryId});
            self = this;
            c.ajax({type: "POST", url: a.client.removeQueryUrl, data: d, dataType: "json", beforeSend: e.beforeSend, success: function(g) {
                    f.setId(null);
                    f.clear(true);
                    if (e.success) {
                        e.success(g)
            ...
            if (e.query) {
                ...
                var selected_fields=$('#selected_fields').val();
                var selectedOperators=$("#selectedOperators").val();
                if(sql!==''){
                var d = this._addAntiForgeryToken({querySQL: sql,selected_fields: selected_fields,selectedOperators:selectedOperators, queryName: g, queryId: f,mapName:"",queryJson: e.query.toJSON()});
                self = this;
                {$("#dialog-form").show()
                    $("#dialog-form").dialog({
                        autoOpen: true,
                        ...
                        buttons: {
                            "Ok": function() {
                                ...
                                var text2 = $("#txt2");
                                 text1.val(text2.val());
                                 if(text2.val()===''){
                                     alert('Enter valid name for map.')
                                 }else{
                                 d['mapName']=text2.val();
                                 c.ajax({type: "POST", url: a.client.saveQueryUrl, data: d,
                                    success: function(h) {
                                        if (h.query) {
            ...
            }else {alert('Generate a query before Save.')}
            }
            ...
            if (f.query) {
                var e = f.options || {};
                var d = this._addAntiForgeryToken({queryJson: f.query.toJSON(), optionsJson: JSON.stringify(e)});
                c.ajax({type: "POST", url: a.client.buildQueryUrl, data: d, dataType: "json", beforeSend: f.beforeSend, success: function(g) {
                        if (f.success) {
                            f.success(g)
                        }
                    }, error: function(g, i, h) {
        ...
        syncQuery: function(f) {
            ...
            if (f.query) {
                ...
                var d = this._addAntiForgeryToken({queryJson: f.query.toJSON(), optionsJson: JSON.stringify(e)});
                var d = this._addAntiForgeryToken({queryJson: f.query.toJSON(), optionsJson: JSON.stringify(e)});
                c.ajax({type: "POST", url: a.client.syncQueryUrl, data: d, dataType: "json", beforeSend: f.beforeSend, success: function(h) {

                        pvt.loadModel({modelId: 'NWind',modelName: 'NWind' ,success:function(a){
                               
                        }})

                    $('#sqlstatement').val(JSON.stringify(h.statement));
                    $('#loadSql').val(f.query.toJSON());
                    $('#selected_fields').val(JSON.stringify(h.selected_fields))
                        if (f.success) {
                            f.success(h)
                        }
                    }, error: function(h, j, i) {
            ...
            var f = g.options || {};
            var sql = $('#sqlstatement').val();
            var e = this._addAntiForgeryToken({SqlQuery: sql,//queryJson: g.query.toJSON(),
                optionsJson: JSON.stringify(f)});
            c.ajax({type: "POST", url: d.executeQueryUrl, data: e, dataType: "json", beforeSend: g.beforeSend, success: function(h) {
                    if (g.success) {
                        g.success(h)
                    }
                }, error: function(h, j, i) {
...
(function(a, b) {
    a.widget("eqjs.ConditionRow", {_condition: null, _parentPredicate: null, _parentPredicateWidget: null, _buttonsBlock: null, _enableButton: null, _deleteButton: null, _addConditionButton: null, _addPredicateButton: null, _checkBox: null, _keepShowingButtons: false, _isMouseOverBlock: false, _active: false, options: {queryPanel: null}, getQuery: function() {
            ...
            this.getQuery().fireChangedEvent({source: d.options.queryPanel, changeType: "condition." + c, condition: f, element: e || d.element})
        }, remove: function() {
            var c = this;
            ...
            e._addConditionButton = a("<div></div>").addClass("eqjs-qp-condition-button eqjs-qp-condition-button-addCondition").attr("title", EQ.core.getText("ButtonAddCondition")).appendTo(c).click(function(f) {
                ...
                e.options.queryPanel.showEntitiesMenu({anchor: e._addConditionButton, selectedIds: null, itemSelectedCallback: function(g, j) {
            ...
            if (f.options.queryPanel.options.showConjunctions && !f._isFirstConditionInGroup()) {
                ...
                if (d) {
                    if(f._condition.expressions!== null && f._condition.expressions!== undefined){
                    var obj = f._condition.expressions[0];
                    var operators = {};
                    var vlere = $("#selectedOperators").val();
                    var idLength = $("#" + f._condition.expressions[0].id).length;
                    var selectId = f._condition.expressions[0].id + '' + idLength;
                    if (vlere !== '' && vlere !== null && vlere!=='null')
                        var operators = jQuery.parseJSON(vlere);
                    if (typeof operators[selectId] !== 'undefined' && operators[selectId] !== null) {
                        d = operators[selectId];
                    }
                    if (d === 'and')
                        var op = 'or';
                    else
                    ...
                    f._conjuction = a("<span id='" + f._condition.expressions[0].id + "'><select  id='mySelect" + selectId + "' onchange='myFunction(" + selectId + "," + JSON.stringify(obj) + ")'><option value=" + d + ">" + d + "</option><option value='" + op + "'>" + op + "</option></select></span>").addClass("eqjs-qp-condelement eqjs-qp-condition-conjuction").prependTo(e)
                    obj['ConjOp'] = d;
                    }else
...
(function(a, b) {
    a.widget("eqjs.ConditionRow_SMPL", a.eqjs.ConditionRow, {_refreshByCondition: function() {
            ...
            for (var l = 0; l < k; l++) {
                ...
                if (o.type === "operator") {
                    if (this._condition.enabled !== false && !this._condition.readOnly) {
                        ...
                        g = a("<a></a>", {href: "javascript:void(0)", text: o.text})
                    } else {
                        g = a("<span></span>", {text: o.text})
                    }
                    var c = a("<div></div>");
            ...
            if (this._condition.enabled !== false && !this._condition.readOnly) {
                ...
                c.click(function(l) {
                    l.preventDefault();
                    k.options.queryPanel.showEntitiesMenu({anchor: c, selectedIds: null, itemSelectedCallback: function(m, q) {
            ...
            var f;
            if (d.kind === "Query") {
                f = {typeName: "QUERY", dataType: d.dataType, kind: d.kind, value: EQ.core.getEmptyQueryObject(), text: ""}
            } else {
                if (d.kind === "Attribute") {
                    ...
                    var h = c ? c.id : null;
                    f = {typeName: "ENTATTR", dataType: d.dataType, kind: d.kind, id: h, value: h, text: ""}
                } else {
                    ...
                    f = {typeName: "CONST", dataType: d.dataType, kind: d.kind, value: g, text: ""}
                }
            }
            return f
            ...
            if (f) {
                ...
                for (var i = 0; i < c; i++) {
                    ...
                    if (d && !(j.options.queryPanel.options.isSubQuery && d.kind === "Query")) {
                        ...
                        g.push({id: d.id, text: k})
                    }
                }
            }
            var h = a("<div></div>").appendTo(e.parent()).hide();
            h.PopupMenu({items: g, onMenuItemSelected: function(l, n) {
...
(function(a, b) {
    a.widget("eqjs.ConditionRow_PDCT", a.eqjs.ConditionRow, {_predicateRowBlock: null, _getButtonsContainer: function() {
            ...
            if (f < 0) {
                ...
                var j = a("<div></div>");
                j.PredicateLinkType({queryPanel: k.options.queryPanel, conditionWidget: k, onChange: d});
            ...
            var k = function(i, q) {
                ...
                return{justAdded: true, typeName: "SMPL", enabled: true, operatorID: q, expressions: [{kind: "Attribute", typeName: n.options.queryPanel.options.attrClassName, id: i.id , module:i.module}]}
            };
            var g = n.getModel();
            if (a.isArray(e)) {
                m = [];
            ...
            var i = {justAdded: true, typeName: "SMPL", enabled: true, operatorID: e, expressions: [{typeName: f.options.queryPanel.options.attrClassName, id: g, kind: "Attribute"}]};
            var d = f._condition.linkType === "All" ? "Any" : "All";
            var c = {typeName: "PDCT", linkType: d, conditions: []};
            c.conditions.push(i);
            this.addCondition(c)
        }, _showButtons: function() {
            if (this._condition.enabled != false) {
...
(function(a, b) {
    a.widget("eqjs.RootPredicate", a.eqjs.ConditionRow_PDCT, {_showButtons: function() {
            if (this._addConditionButton) {
                this._addConditionButton.show()
            }
            if (this._addPredicateButton) {
...
(function(a, b) {
    a.widget("eqjs.QueryPanel", {_activeConditionWidget: null, _rootPredicateWidget: null, _query: null, _defaultMenuOptions: {showSearchBoxAfter: 30, activateOnMouseOver: true}, options: {isSubQuery: false, activeCondition: null, listRequestHandler: null, entitiesPopupHandler: null, entitiesListFilter: null, menuContainer: document.body, showRootRow: true, showAddRow: true, showCheckboxes: false, showPoweredBy: true, alwaysShowButtonsInPredicates: false, alwaysShowButtonsInConditions: false, showConjunctions: true, accentActiveCondition: true, activateRootOnStart: true, dateFormatValue: "", dateFormatDisplay: "d MM, yy", attrElementFormat: "{entity} {attr}", menuOptions: {showSearchBoxAfter: 30, activateOnMouseOver: true}, subQueryDialogWidth: 600, subQueryDialogHeight: 300, dialogZIndex: 100000, numberDecimalSeparatorDisplay: ".", numberListSeparators: [",", ";"], autoEditNewCondition: false, attrClassName: "ENTATTR", defaultQuery: {root: {linkType: "All", enabled: true, conditions: []}, columns: [], justsorted: []}}, addRowHeight: 0, getSelf: function() {
            ...
            if (!e.isModelNullOrEmpty()) {
                e.options.entitiesList = e.getEntitiesList({addUIC: true, addUIR: false, addUIS: false});
                ...
                if (e.options.showAddRow) {
                    ...
                    h.click(function(i) {
                        i.preventDefault();
                        e.showEntitiesMenu({anchor: h, selectedIds: null, itemSelectedCallback: function(j, l) {
            ...
            e.element.droppable({hoverClass: "eqjs-drophover", scope: "entityAttr", drop: function(i, j) {
            ...
            if (e.options.showPoweredBy || poweredByOption.show === true) {
                var c = a("<a></a>", {text: "", href: "", target: "_blank", css: {color: "#4676AE", font: "11px Calibri", "text-decoration": "underline"}}).appendTo(e.element);
                var f = function() {
                    c.css({position: "absolute", bottom: "-15px", right: "0px"});
            ...
            c.RootPredicate({queryPanel: this, model: this.getModel()});
            ...
            var d = this;
            var e = this.getQuery();
            e.fireChangedEvent({source: d, changeType: "query." + c})
        }, _fireEntitiesPopup: function(d) {
            var c = this;
            if (this.options.entitiesPopupHandler) {
                return this.options.entitiesPopupHandler({query: c.getQuery(), items: d.items})
            } else {
                return false
            }
            ...
            var e = d._fireEntitiesPopup({items: c});
            if (e) {
            ...
            var d = {items: c.options.entitiesList};
...
(function(a, b) {
    a.widget("eqjs.ValueEditor", {_expr: {value: "", text: ""}, _linkElement: null, options: {model: null, conditionWidget: null, editor: null, onChange: null}, getModel: function() {
            ...
            if (d.enabled !== false && !d.readOnly) {
                c._linkElement = a("<a></a>", {href: "javascript:void(0)", text: "-"}).appendTo(c.element);
                c._linkElement.click(function() {
                    c.showEditor();
                    return false
                })
            } else {
                c._linkElement = a("<span></span>", {text: "-"}).appendTo(c.element)
            }
        }, showEditor: function() {
...
(function(a, b) {
    a.widget("eqjs.ValueEditor_EDIT", a.eqjs.ValueEditor, {_numberDecimalSeparatorValue: ".", _numberListSeparatorValue: ",", _numberDecimalSeparatorDisplay: ".", _numberListSeparators: [",", ";"], _editBox: null, _editBoxClass: "eqjs-qp-ve-editbox", _create: function() {
            var c = this.getQueryPanel();
            if (c) {
            ...
            var c = this;
            var d = false;
            c._editBox = a("<input />", {type: "text", "class": c._editBoxClass, blur: function(f) {
            ...
            if (isNaN(c) || (EQ.core.isNumericType(this._expr.dataType) && (a.trim(d) != d))) {
                return{result: false, message: '"' + d + '"' + this.getResText("ErrNotNumber")}
            } else {
                if (EQ.core.isIntType(this._expr.dataType) && c != parseInt(d, 10)) {
                    return{result: false, message: d + " - " + this.getResText("ErrIncorrectInteger")}
                }
            }
            return{result: true, message: ""}
            ...
            if (EQ.core.isNumericType(this._expr.dataType)) {
                if (f.length > 0 && this._expr.kind == "List") {
                    ...
                    for (var c = 0; c < e; c++) {
                        ...
                        if (!d || !d.result) {
                            return{result: false, message: this.getResText("ErrIncorrectNumberList")}
                        }
                    }
                    return{result: true}
                } else {
...
(function(a, b) {
    a.widget("eqjs.ValueEditor_DATETIME", a.eqjs.ValueEditor_EDIT, {_dateFormatInternal: "yy-mm-dd", _dateFormatValue: "mm/dd/yy", _dateFormatDisplay: "d mm, yy", _dateValue: "", _timeFormatInternal: "HH:mm", _timeFormatValue: "HH:mm", _timeFormatDisplay: "HH:mm", _create: function() {
            var c = this.getQueryPanel();
            if (c) {
            ...
            var c = this;
            c._editBox = a("<input />", {type: "text", "class": "eqjs-qp-ve-editbox"}).appendTo(c.element).hide();
            if (c._expr.dataType == "Time") {
                c._editBox.timepicker({onClose: function(g, f) {
                        if (g != "") {
                            var d = c.getModel();
                            var e = f.lastVal;
                ...
                if (c._expr.dataType == "Date") {
                    c._editBox.datepicker({changeMonth: true, changeYear: true, beforeShow: function() {
                            c._editBox.show()
                            ...
                            if (h != "") {
                                ...
                                if (d != c._convertDateString(f, c._dateFormatValue, c._dateFormatInternal)) {
                                    c._setValue(d)
                                }
                                c._editBox.hide()
                            } else {
                    ...
                    if (c._expr.dataType == "DateTime") {
                        c._editBox.datetimepicker({changeMonth: true, changeYear: true, beforeShow: function() {
                                c._editBox.show()
                            }, onClose: function(m, i) {
                                if (m != "") {
                                    var j = c.getModel();
                                    var k = i.lastVal.split(" ");
                                    var e = "";
                                    var d = "";
                                    e = k[0];
                                    if (k.length > 1) {
                                    ...
                                    var n = c._convertDate(a(this).datepicker("getDate"), c._dateFormatInternal);
                                    var l = c._convertTime(a(this).datepicker("getDate"), c._timeFormatInternal);
                                    var h = "";
                                    var g = "";
                                    var f = c._expr.value.split(" ");
                                    if (n != c._convertDateString(e, c._dateFormatValue, c._dateFormatInternal)) {
                                        h = n
                                    } else {
                                        h = f[0]
                                    }
                                    if (l != c._convertTimeString(d, c._timeFormatValue, c._timeFormatInternal)) {
                                        g = l
                                    } else {
                                        if (f.length > 1) {
                                            g = f[1]
                                    ...
                                    if (a.trim(h + " " + g) != c._expr.value) {
                                        c._setValue(a.trim(h + " " + g))
                                    }
                                    c._editBox.hide()
                                } else {
                                    c._editBox.hide();
            ...
            o._editBox.css({left: h, top: m, width: o._expr.dataType == "DateTime" ? "150px" : "115px", "min-width": o._linkElement.width(), position: "absolute", zIndex: n}).show().focus()
            ...
            var d = {hour: c.getHours(), minute: c.getMinutes(), second: c.getSeconds(), millisec: c.getMilliseconds(), microsec: c.getMicroseconds(), timezone: c.getTimezoneOffset() * -1};
...
(function(a, b) {
    a.widget("eqjs.ValueEditor_LIST", a.eqjs.ValueEditor, {_menuBlock: null, _menuItemsList: null, _emptyListText: "<empty list>", _showWhenReady: false, _getEmptyText: function() {
            ...
            var d = {items: c._menuItemsList, multiselect: f, container: e.options.menuContainer, onMenuItemSelected: function(g, l) {
                    if (!f) {
            ...
            var c = this;
            if (c._menuBlock) {
                this._menuBlock.PopupMenu("showMenu", {anchor: c._linkElement, selectedIds: c._getValue()})
            } else {
                this._showWhenReady = true
            }
        }, _adjustExprText: function() {
...
(function(a, b) {
    a.widget("eqjs.ValueEditor_CUSTOMLIST", a.eqjs.ValueEditor_LIST, {_loaderElement: null, _renderEditor: function() {
            var c = this;
            this._linkElement.hide();
            if (!this._loaderElement) {
                this._loaderElement = a("<div></div>", {"class": "eqjs-qp-ve-loader"}).appendTo(c.element)
            }
            ...
            var d = this, e = this._getListName(), c = this.getQuery().getListRequestHandler();
...
(function(a, b) {
    a.widget("eqjs.ValueEditor_SQLLIST", a.eqjs.ValueEditor_CUSTOMLIST, {_fillMenuItemsList: function(g) {
            var d = this;
            var f = this.getQuery();
            var e = this.options.editor;
            var c = f.getListRequestHandler();
            if (c) {
                c({listName: "SQL", sql: e.sql, editorId: e.id}, function(h) {
                    d._menuItemsList = h;
                    if (g) {
                        g()
                    }
...
(function(a, b) {
    a.widget("eqjs.ValueEditor_SUBQUERY", a.eqjs.ValueEditor, {_dialogBlock: null, _queryPanelBlock: null, _columnElement: null, _getEmptyText: function() {
            return this.getResText("MsgSubQueryValue")
        }, _renderEditor: function() {
            var k = this;
            k._dialogBlock = a("<div></div>", {"class": "eqjs-qp-ve-subquery"}).hide().appendTo(k.element);
            ...
            c.PopupMenu({items: g, zIndex: i.options.dialogZIndex + 1000});
            ...
            k._columnElement.click(function(l) {
                c.PopupMenu("showMenu", {anchor: k._columnElement, selectedIds: null, itemSelectedCallback: function(m, n) {
            ...
            k._dialogBlock.dialog({autoOpen: false, draggable: false, resizable: false, title: k.getResText("SubQueryDialogTitle"), dialogClass: "eq-js-dialog", closeText: "X", modal: true, width: i.options.subQueryDialogWidth, minHeight: i.options.subQueryDialogHeight, zIndex: i.options.dialogZIndex, buttons: [{name: "eqjs-subquery-button-ok", text: k.getResText("ButtonOK"), click: function() {
                            var m = k.getSubQuery();
                            k._expr.value = m.getObject();
                            a(this).dialog("close");
                            var l = k.getQuery();
                            l.fireChangedEvent({source: k.getQueryPanel(), changeType: "condition.changed", condition: k.getCondition()})
                        }}, {name: "eqjs-subquery-button-cancel", text: k.getResText("ButtonCancel"), click: function() {
                            a(this).dialog("close")
                        }}], open: function() {
            ...
            if (c) {
                ...
                if (h.length > 0) {
                    ...
                    var e = {caption: "", sorting: "None", sortIndex: -1, expr: {typeName: "ENTATTR", id: g, kind: "Attribute", dataType: c.dataType}};
                    h.push(e)
                }
            ...
            d._queryPanelBlock.QueryPanel({isSubQuery: true, showRootRow: c.options.showRootRow, showAddRow: true, menuContainer: d._dialogBlock.get(0), showCheckboxes: c.options.showCheckboxes, dateFormatValue: c.options.dateFormatValue, dateFormatDisplay: c.options.dateFormatDisplay, entitiesListFilter: function(l, k) {
...
(function(a, b) {
    a.widget("eqjs.PredicateLinkType", a.eqjs.ValueEditor_LIST, {_predicate: null, options: {queryPanel: null}, getCondition: function() {
            ...
            var c = this;
            c._menuBlock.PopupMenu("showMenu", {anchor: c._linkElement})
        }, _fillMenuItemsList: function() {
            ...
            var d = this, c = this._getEmptyText();
...
(function(a, b) {
    a.widget("eqjs.ColumnsContainer", {_activeColumnWidget: null, _defaultMenuOptions: {showSearchBoxAfter: 30, activateOnMouseOver: true}, _query: null, options: {isSubQuery: false, activeColumn: null, showAddRow: true, showHeader: true, showColumnCaptions: true, allowAggrColumns: true, allowSorting: true, attrElementFormat: "{entity} {attr}", alwaysShowButtons: false, accentActiveColumn: true, menuSearchBoxAfter: 30, allowDragDrop: true, menuOptions: {showSearchBoxAfter: 30, activateOnMouseOver: true, adjustHeight: true}}, _create: function() {
            ...
            if (k && !k.isEmpty()) {
                n.options.entitiesList = k.getEntitiesTree({addUIC: false, addUIR: true, addUIS: false});
                n.options.entitiesMenu = n._createEntitiesMenu();
                if (n.options.allowSorting != false) {
                    n.options.sortMenuList = [{id: "None", text: EQ.core.getText("CmdNotSorted")}, {id: "Ascending", text: EQ.core.getText("CmdAscending")}, {id: "Descending", text: EQ.core.getText("CmdDescending")}, {id: "---", text: "---"}, {id: "MoveTop", text: EQ.core.getText("CmdMoveToFirst")}, {id: "MoveUp", text: EQ.core.getText("CmdMoveToPrev")}, {id: "MoveDown", text: EQ.core.getText("CmdMoveToNext")}, {id: "MoveBottom", text: EQ.core.getText("CmdMoveToLast")}];
                ...
                if (n.options.showAddRow) {
                    ...
                    j.appendTo(d);//click add new
                    j.click(function(o) {
                        o.preventDefault();
                       n.showEntitiesMenu({anchor: j, selectedIds: null, itemSelectedCallback: function(q, t) {
                                var s = t.menuItem.id;
                ...
                if (l) {
                    l.css({"max-height": n.element.innerHeight() - m + 5 + "px"})
                }
            }
        }, _updateLists: function() {
        }, _setOption: function(c, d) {
            ...
            var k, r, c, e = false, n, q = false;
            var m = t.getModel();
            if (t.options.allowDragDrop) {
                g.sortable({containment: h, tolerance: "touch", cancel: "." + j + "-column-buttonsBlock, ." + j + "-colelement, ." + j + "-sortbutton", placeholder: "ui-state-highlight eqjs-highlight", scrollSpeed: 3, delay: 100, distance: 13, cursorAt: {top: 10}, forceHelperSize: true, start: function(u, v) {
                        k = v.item.index();
                ...
                t.element.droppable({hoverClass: "eqjs-drophover", scope: "entityAttr", drop: function(u, w) {
            ...
            var d = {items: this.options.entitiesList, adjustHeight: c.options.adjustEntitiesMenuHeight};
            a.extend(d, c.options.menuOptions);
            f.PopupMenu(d);
            return f
            ...
            var c = this;
            e.PopupMenu({items: this.options.sortMenuList});
            return e
        }, _fireQueryChange: function(c) {
            var d = this;
            this.getQuery().fireChangedEvent({source: d, changeType: "query." + c})
        }, _fireColumnChange: function(c, e) {
            var d = this;
            e = e || d._column;
            this.getQuery().fireChangedEvent({source: d, changeType: "column." + c, column: e})
            ...
            var k = function(i) {
                ...
                return{caption: "", sorting: "None", sortIndex: -1, expr: {typeName: "ENTATTR", id: i}}
            };
            if (!this.getQuery()) {
                return
            }
    ...
    a.widget("eqjs.ColumnsPanel", a.eqjs.ColumnsContainer, {_getCssPrefix: function() {
            return"eqjs-qc"
        }});
    a.widget("eqjs.ColumnsBar", a.eqjs.ColumnsContainer, {_create: function() {
...
(function(a, b) {
    a.widget("eqjs.ColumnRow", {_column: null, _buttonsBlock: null, _columnTypeButton: null, _deleteButton: null, _keepShowingButtons: false, _isMouseOverBlock: false, _sortingButton: null, _panelCssPrefix: "eqjs-qc", _classesToAdd: "", _active: false, options: {columnsPanel: null}, getQuery: function() {
            ...
            e = e || d._column;
            var f = this.getQuery();
            f.fireChangedEvent({source: d.options.columnsPanel, changeType: "column." + c, column: e})
        }, refresh: function() {
            this._render()
        }, _setOption: function(c, d) {
            if (arguments.length == 2) {
            ...
            if (g.options.columnsPanel.options.allowSorting != false) {
                g._sortingButton.attr("title", EQ.core.getText("ButtonSorting")).click(function() {
                    ...
                    g.options.columnsPanel.options.sortMenu.PopupMenu("showMenu", {anchor: g._sortingButton, selectedIds: null, itemSelectedCallback: function(h, i) {
...
(function(a, b) {
    a.widget("eqjs.ColumnRow_ENTATTR", a.eqjs.ColumnRow, {_baseAttr: null, _menuFunctionsBlock: null, _initCssPrefixes: function() {
            ...
            if (c._column.enabled !== false && !c._column.readOnly) {
                d = a("<a></a>").attr("href", "javascript:void(0)").text(c._getAttributeText(c._baseAttr)).click(function(f) {
                    f.preventDefault();
                    c.options.columnsPanel.options.entitiesMenu.PopupMenu("showMenu", {anchor: d, selectedIds: null, itemSelectedCallback: function(e, g) {
            ...
            if (c.options.columnsPanel.options.showColumnCaptions) {
                ...
                if (!c._column.readOnly) {
                    ...
                    e.ColumnRow_CaptionEditor({cssClassPrefix: this._panelCssPrefix});
            ...
            c._column.caption = "";
            c._column.expr = {func: f, distinct: false, typeName: "AGGRFUNC", argument: {typeName: "ENTATTR", id: g}};
            var e = c._column;
            var d = c._active;
            c.destroy();
            c._clear();
            c.element.ColumnRow_AGGRFUNC({columnsPanel: c.options.columnsPanel});
            ...
            l._menuFunctionsBlock.PopupMenu({items: f});
            ...
            if (c._columnTypeButton) {
                c._columnTypeButton.click(function(d) {
                    if (c.options.columnsPanel.options.allowAggrColumns !== false && c._column.enabled !== false) {
                        ...
                        c._menuFunctionsBlock.PopupMenu("showMenu", {anchor: c._columnTypeButton, selectedIds: null, itemSelectedCallback: function(e, f) {
...
(function(a, b) {
    a.widget("eqjs.ColumnRow_AGGRFUNC", a.eqjs.ColumnRow_ENTATTR, {_displayFormatParser: {formatStr: "", pos: 0, token: "text", tokenText: "", start: function(c) {
                this.formatStr = c;
                this.pos = 0;
                this.tokenText = ""
            }, skipSpaces: function() {
            ...
            while (e.next()) {
                if (e.token === "function") {
                    c.push({type: "function", text: e.tokenText})
                } else {
                    if (e.token === "attribute") {
                        c.push({type: "attribute"})
                    } else {
                        if (e.token === "text") {
                            c.push({type: "text", text: e.tokenText})
                        }
                    }
                }
            }
            ...
            if (n._column.enabled !== false && !n._column.readOnly) {
                d = a("<a></a>", {href: "javascript:void(0)"}).appendTo(l);
            ...
            for (var h = 0; h < f; h++) {
                ...
                if (m.type === "function") {
                    ...
                    if (n._column.enabled !== false) {
                        d.click(function(o) {
                            n._menuFunctionsBlock.PopupMenu("showMenu", {anchor: d, selectedIds: null, itemSelectedCallback: function(q, r) {
            ...
            c._column.caption = "";
            c._column.expr = {typeName: "ENTATTR", id: f};
            var e = c._column;
            var d = c._active;
            c.destroy();
            c._clear();
            c.element.ColumnRow_ENTATTR({columnsPanel: c.options.columnsPanel});
...
(function(a, b) {
    a.widget("eqjs.ColumnRow_CaptionEditor", a.eqjs.ValueEditor_EDIT, {_column: null, _defaultValue: "", _editBoxClass: "eqjs-qc-ce-editbox", _onChange: null, options: {cssClassPrefix: "eqjs-qc"}, init: function(e, c, d) {
...
(function(a, b) {
    a.widget("eqjs.EntitiesPanel", {_rootEntityBlock: null, _model: null, _progressBlock: null, options: {queryPanelId: "QueryPanel", columnsPanelId: "ColumnsPanel", showToolbar: true, showSelectAllButton: false, showClearSelectionButton: true, showAddColumnButton: true, showAddConditionButton: true, showCheckboxes: true, clickableAttributes: 0, draggableAttributes: true, showAttributes: {usedInConditions: true, usedInColumns: true, usedInSorting: false}, showFilterBox: false, showIndicatorOnLoad: true}, getSelf: function() {
            ...
            var s = this;
            var r = "eqjs-ep-entity", k, g = a("<div></div>", {"class": r + "-node"}), t = a("<div></div>", {"class": r + "-children"}), z = a("<label></label>", {"class": r + "-node-label"}), C = a("<input />", {type: "checkbox"}), e = a("<a></a>", {href: "javascript:void(0)", "class": r + "-node-button"}), o, l, w, y, u, x, h = a("<div></div>"), A = m;
            var c = function(D, i) {
            ...
            if (q.name && q.name != "") {
                ...
                e.prependTo(g);
                for (var w = 0; w < A; w++) {
                    g.prepend(a("<div></div>", {"class": r + "-offset"}))
                }
                k.append(g);
                t.hide();
                A++
                ...
                for (var w = 0; w < f; w++) {
                    ...
                    if (d(u)) {
                        if (!u.lookupAttr || !(c(u.lookupAttr, t) || c(u.lookupAttr, this._rootEntityBlock))) {
                            ...
                            l = a("<label></label>", {text: x, "class": r + "-attr-label"});
                            if (s.options.showCheckboxes) {
                                l.prepend('<input type="checkbox" />')
                            }
                            for (var v = 0; v < A + 1; v++) {
                                l.prepend(a("<div></div>", {"class": r + "-offset"}))
                            }
                            ...
                            if (s.options.draggableAttributes) {
                                o.draggable({containment: "document", cursor: "move", helper: "clone", revert: "invalid", scope: "entityAttr", distance: 20, connectToSortable: ".ui-sortable", cursorAt: {bottom: 20, left: 10}, start: function(i, j) {
            ...
            var d = this;
            var g = "eqjs-ep-filter-box", f = a("<div></div>", {"class": g}), c = a("<input></input>", {"class": g + "-input"});
            f.append(c);
            d.element.prepend(f)
        }, _createToolPanel: function(d) {
            var l = this;
            var f = "eqjs-ep-tool-panel", m = a("<div></div>", {"class": f}), g = a("<div></div>", {"class": f + "-select-all", title: "Select all"}), c = a("<div></div>", {"class": f + "-deselect-all", title: "Clear selection"}), k = a("<div></div>", {"class": f + "-add-columns", title: "Add column"}), e = a("<div></div>", {"class": f + "-add-cond", title: "Add condition"}), j = a("<div></div>", {"class": f + "-left-side"}), i = a("<div></div>", {"class": f + "-right-side"});
            var h = l.getModel();
...
(function(d, g) {
    ...
    d.widget("eqjs.ColumnsBarOld", {_query: null, _activeColumn: null, _activeColumnIndex: null, _contentBlockClass: "eqjs-cb-inner-wrapper", _colsBlockClass: "eqjs-cb-inner", _width: 0, _leftBtn: null, _rightBtn: null, _uiUpdaterId: null, options: {showAddButton: true, addButtonSide: "left", addButtonHTML: "+", innerPaddingCompensation: {right: 38, left: 35}, allowAggrColumns: true, allowSorting: true, sortByList: ["None", "Ascending", "Descending"], placeholderHeight: 22, adjustEntitiesMenuHeight: true, attrElementFormat: "{entity} {attr}", alwaysShowButtons: false, accentActiveColumn: true, uiUpdateInterval: 200, scrollSpeed: 15, showScrollBar: true, scrollButtonsScrollBy: 500, scrollButtonsLeftHTML: "&lt; ...", scrollButtonsRightHTML: "... &gt;"}, _setOption: function(i, j) {
            ...
            var u = this;
            var t = d("<div></div>").addClass(u._colsBlockClass), j = d("<div></div>").addClass(u._contentBlockClass);
            var r = u.getQuery();
            var k = r ? r.getColumns() : null, i = function(x, z, v) {
                var y = k[colIdx];
                var w = z["ColumnsBarColumn_" + y.expr.typeName];
                w.call(z, {columnsBar: x, query: x.getQuery()});
                w.call(z, "init", y);
                return{wfn: w, element: z}
            };
            ...
            if (!u.options.showScrollBar) {
                ...
                u._leftBtn = u._makeScrollButton({html: u.options.scrollButtonsLeftHTML, t: "left", otherT: "right", scrollLeft: "-=" + u.options.scrollButtonsScrollBy});
                u._rightBtn = u._makeScrollButton({html: u.options.scrollButtonsRightHTML, t: "right", otherT: "left", scrollLeft: "+=" + u.options.scrollButtonsScrollBy});
            ...
            u._uiUpdaterId = window.setInterval(function() {
                ...
                if (w > v && w !== u._width) {
                    u._width = w;
                    u.element.find("." + u._colsBlockClass).css({width: w})
                }
            ...
            u.element.droppable({tolerance: "touch", hoverClass: "eqjs-drophover", scope: "entityAttr", drop: function(v, w) {
            ...
            var j = this;
            i.click(function() {
                j.showEntitiesMenu({anchor: i, selectedIds: null, itemSelectedCallback: function(m, q) {
                        var o = q.menuItem.id;
            ...
            i.PopupMenu({items: this.options.entitiesList, adjustHeight: this.options.adjustEntitiesMenuHeight});
            return i
        }, _fireQueryChange: function(i) {
            var j = this;
            this.getQuery().fireChangedEvent({source: j, changeType: "query." + i})
        }, _fireColumnChange: function(i, k) {
            var j = this;
            k = k || j._column;
            this.getQuery().fireChangedEvent({source: j, changeType: "column." + i, column: k})
    ...
    var a = [{id: "|<-", text: EQ.core.getText("CmdMoveToStart"), fn: function(i) {
                c(h(i, 0, i.self.element.index()), i.self.element)
            }}, {id: "->", text: EQ.core.getText("CmdMoveRight"), fn: function(j) {
            ...
            }}, {id: "<-", text: EQ.core.getText("CmdMoveLeft"), fn: function(i) {
            ...
            }}, {id: "->|", text: EQ.core.getText("CmdMoveToEnd"), fn: function(i) {
                var k = i.self.element;
    ...
    d.widget("eqjs.ColumnsBarColumn", {_column: null, _sortingButton: null, _buttonsBlock: null, _menuButton: null, _menuButtonMenu: null, _deleteButton: null, _keepShowingButtons: false, _isMouseOverBlock: false, _classesToAdd: "", _active: false, options: {columnsBar: null, query: null}, getQuery: function() {
            ...
            l.fire({query: j.options.columnsBar.options.query, changeType: "column." + i, condition: k})
        }, refresh: function() {
            this._render()
        }, _setOption: function(i, j) {
            if (arguments.length == 2) {
            ...
            this._menuButtonMenu.PopupMenu({items: j});
            var i = this;
            i._menuButton.click(function() {
                i._menuButtonMenu.PopupMenu("showMenu", {anchor: i._menuButton, selectedIds: null, itemSelectedCallback: function(l, m) {
                        k(j, m.menuItem.id, {self: i, evt: l, data: m});
                        return false
                    }, menuClosedCallback: function() {
    ...
    var e = function(i) {
        return[{id: "sep", text: "-------"}].concat(i)
    };
    d.widget("eqjs.ColumnsBarColumn_ENTATTR", d.eqjs.ColumnsBarColumn, {_classesToAdd: "eqjs-cb-column eqjs-cb-column-entattr", _baseAttr: null, _menuFunctionsBlock: null, _getMenuButtonMenuItems: function() {
            ...
            i._column.expr = {func: l, distinct: false, typeName: "AGGRFUNC", argument: {typeName: "ENTATTR", id: m}};
            var k = i._column;
            var j = i._active;
            i.destroy();
            i._clear();
            i.element.ColumnsBarColumn_AGGRFUNC({columnsBar: i.options.columnsBar, model: i.getModel()});
    ...
    d.widget("eqjs.ColumnsBarColumn_AGGRFUNC", d.eqjs.ColumnsBarColumn_ENTATTR, {_classesToAdd: "eqjs-cb-column eqjs-cb-column-aggr", _getMenuButtonMenuItems: function() {
            return[{id: "to-simple", text: EQ.core.getText("CmdToSimple"), fn: function(i) {
        ...
        }, _displayFormatParser: {formatStr: "", pos: 0, token: "text", tokenText: "", start: function(i) {
                this.formatStr = i;
                this.pos = 0;
                this.tokenText = ""
            }, skipSpaces: function() {
            ...
            while (k.next()) {
                if (k.token === "function") {
                    i.push({type: "function", text: k.tokenText})
                } else {
                    if (k.token === "attribute") {
                        i.push({type: "attribute"})
                    } else {
                        if (k.token === "text") {
                            i.push({type: "text", text: k.tokenText})
                        }
                    }
                }
            }
            ...
            i._column.caption = "";
            i._column.expr = {typeName: "ENTATTR", id: l};
            var k = i._column;
            var j = i._active;
            i.destroy();
            i._clear();
            i.element.ColumnsBarColumn_ENTATTR({columnsBar: i.options.columnsBar, model: i.getModel()});
    ...
    d.widget("eqjs.ColumnsBarColumn_CaptionEditor", d.eqjs.ValueEditor_EDIT, {_column: null, _defaultValue: "", _editBoxClass: "eqjs-ce-editbox", _onChange: null, init: function(k, i, j) {
            ...
            i._linkElement = d("<span></span>", {text: "-"}).appendTo(i.element);
...
(function(a, b) {
    a.widget("eqjs.FilterBar", {_queryPanelWidget: null, _query: null, _queryIsChanged: false, _headerElement: null, _headerIconElement: null, _headerTextBlock: null, _headerTextElement: null, _headerLinkElement: null, _headerArrowBlock: null, _headerArrowElement: null, _qpBlock: null, _qpElement: null, _buttonsBlock: null, _btnApply: null, _btnClear: null, options: {applyOnClose: true, showApplyButton: true, showClearButton: true, useBootstrapIcons: true, applyFilterCallback: null, queryPanel: {showAddRow: true, showCheckboxes: true, alwaysShowButtonsInPredicates: true}}, getQuery: function() {
            return this._query
        }, setQuery: function(c) {
            if (this._query) {
            ...
            this._headerLinkElement = a('<a href="javascript:void(0)">' + EQ.core.getText("StrNoFilterClickToAdd") + "</a>").addClass("eqjs-fb-header-link").hide().click(function() {
                c._queryPanelWidget.showEntitiesMenu({anchor: c._headerLinkElement, selectedIds: null, itemSelectedCallback: function(d, f) {
                        var e = f.menuItem.data("id");
...
(function($, window) {
    ...
    EQ.view = {EqDataTable: function(data) {
        ...
        }, beforeTableRendering: null, formatColumnHeader: null, formatGridCell: null, applyCommonOptions: function(options) {
    ...
    EQ.view.EqDataTable.prototype = {getNumberOfColumns: function() {
...
function changeQuery(mapid) {
    if ($('#check' + mapid).is(":checked") === true) {
        $('#loadSql').val($('#map' + mapid).val());
        $('#selectedOperators').val($('#mapOp' + mapid).val());
        $('#mapid').val(mapid);
...
function myFunction(id, objekti) {
    operators = {};
    var val = $("#selectedOperators").val();
    if (val !== '' && val !== 'null')
        var operators = jQuery.parseJSON(val);
    var x = document.getElementById("mySelect" + id).value;
    $("#mySelect" + id).val(x);
    // $("#mySelect"+id +" option[value="+x+"]").attr('selected','selected');
    operators[id] = x;
    $("#selectedOperators").val(JSON.stringify(operators));
}
function updateQuery() {
    var sqlstatement = $("#sqlstatement").val();
    var loadJson = $("#loadSql").val();
    var mapid = $('#mapid').val();
    var selected_fields = $('#selected_fields').val();
    var selectedOperators = $("#selectedOperators").val();
  
    var elementFound=$('.div.eqjs-qc-colelement,.eqjs-qc-captionelement').text();
    if(elementFound!==''){
    
    if (mapid !== '' && sqlstatement !== '') {
        var url='index.php?module=QueryBuilder&action=QueryBuilderAjax&file=QueryBuilder&query_action=updateQuery';
        var d = {queryJson: loadJson, selected_fields: selected_fields, mapid: mapid , querySQL:sqlstatement,selectedOperators:selectedOperators };
        $.ajax({type: "POST", url: url, data: d, success: function(h) {
                alert('Query updated!');
            },
            error: function(h){
                alert("Errors while updating query!");
            }
        });
    }
    else
        {
            alert('This query was not found. Use "Save As Query" button to create a new query.')
        }
    }else{
