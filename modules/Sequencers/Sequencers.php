...
class Sequencers extends CRMEntity {
	 ...
	 */
	 function save_related_module($module, $crmid, $with_module, $with_crmid) { 
		...
		$classPlannedActions = CRMEntity::getInstance('PlannedActions');
		if ($with_module == 'PlannedActions') {
			foreach ($with_crmid as $relcrmid) {
				$query = "select distinct recipient_id from vtiger_palaunch where sequencer_id={$crmid}";
				$res = $adb->query($query);
				while ($row=$adb->getNextRow($res)) {
					$recipientId = $row['recipient_id'];
					$type = getSalesEntityType($recipientId);
					foreach ($with_crmid as $relcrmid) {
						$classPlannedActions->save_related_module('PlannedActions', $relcrmid, $type, $recipientId, $crmid);
					}
				}
			}
			parent::save_related_module($module, $crmid, $with_module, $with_crmid);
		} else {
			$query = "select relcrmid from vtiger_crmentityrel where crmid={$crmid} and module='Sequencers' and relmodule='PlannedActions'";
			$res = $adb->query($query);
			while ($row=$adb->getNextRow($res)) {
				$plannedActionId = $row['relcrmid'];
				foreach ($with_crmid as $contactId) {
					$type = getSalesEntityType($contactId);
					$classPlannedActions->save_related_module('PlannedActions', $plannedActionId, $type, $contactId, $crmid);
				}
			}
		}
		...
		if ($with_module == 'PlannedActions') {
			foreach ($with_crmid as $relcrmid) {
				...
				while ($row=$adb->getNextRow($res)) {
					$recipientId = $row['recipient_id'];
					$type = getSalesEntityType($recipientId);
					foreach ($with_crmid as $plannedActionId) {
						$classPlannedActions->delete_related_module('PlannedActions', $relcrmid, $type, $recipientId, $crmid);
					}
				}
			}
			...
			while ($row=$adb->getNextRow($res)) {
				...
				foreach ($with_crmid as $contactId) {
					$type = getSalesEntityType($contactId);
					$classPlannedActions->delete_related_module('PlannedActions', $plannedActionId, $type, $contactId, $crmid);
				}
			}
		}
