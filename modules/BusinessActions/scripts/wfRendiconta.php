...
function wfRendiconta($entity)
{
 ...
 $query=$adb->pquery("select posizione,statopraticacorriere 
      from vtiger_project 
      where projectid=? ",array($projectid));
 $posizione=$adb->query_result($query,0,'posizione');
 $data_to_be_passed=array('recordid'=>$projectid,'posizione'=>$posizione);
