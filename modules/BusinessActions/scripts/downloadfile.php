...
function downloadfile($param){
    ...
    global $adb;
global $fileId, $default_charset, $app_strings,$site_URL;

$attachmentsid = vtlib_purify($param['fileid']);
$entityid = vtlib_purify($param['entityid']);
...
if(!empty($deletecheck) && $adb->query_result($deletecheck, 0, 'deleted') == 1) {
	...
	if($adb->num_rows($result) == 1)
	{
		...
		//$fileContent = fread(fopen($filepath.$saved_filename, "r"), $filesize);
                $resp['pdfURL']=$site_URL."/".$filepath.$saved_filename;
                return $resp;
