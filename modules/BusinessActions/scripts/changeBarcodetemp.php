...
$mapid=$_REQUEST['map'];
 ...
 $record = $recordid;//$importantStuff[1];
    ...
    if($tnterror=='1'){
        ...
        if(file_exists("/var/www/eprice/storage/ModuloDiReso_$recordid.pdf")){
            $pagecount = $pdf->setSourceFile("/var/www/eprice/storage/ModuloDiReso_$recordid.pdf");
        }
        elseif(file_exists("/var/www/eprice/storage/sovracollo_$recordid.pdf")){
            $pagecount = $pdf->setSourceFile("/var/www/eprice/storage/sovracollo_$recordid.pdf");
        }
        else{
            $pagecount = $pdf->setSourceFile("/var/www/eprice/storage/Etichette_Reso_$recordid.pdf");
        }
        $tpl = $pdf->importPage(1);
        $pdf->useTemplate($tpl);       

        $pdf->Output("/var/www/eprice/storage/Etichette_Reso_$recordid.pdf", "I");
        $pdf->Output("/var/www/eprice/storage/Etichette_Reso_$recordid.pdf", "F");
    }
...
//            $adb->pquery("Update vtiger_project"
//                    . " set tnterror=?"
//                    . " where vtiger_project.projectid=?",array('1',$record));
//            include 'modules/Actions/scripts/changeBarcodetempMessage.php';
//            generate_barcode($record,$arr_tnt);
//            include 'modules/Actions/scripts/changeBarcodetempMessage.php';
//            generate_barcode($record,$arr_tnt);
...
$p3",0,'L');
 ...
 if($lockindex<=4){
        ...
        for($c_p=0;$c_p<$nr_pd;$c_p++){
            ...
            shell_exec("cp /var/www/eprice/storage/Etichette_Reso_$record.pdf /var/www/eprice/storage/Etichette_Reso_$projectid.pdf");
        }
