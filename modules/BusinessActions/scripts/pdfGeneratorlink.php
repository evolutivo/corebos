...
WHERE adoctomaster =$recid[0] and deleted=0";
 ...
 $docSettings="select * from vtiger_docsettings join vtiger_crmentity on crmid=docsettingsid where causale='$b' and partnerprimario='$pp' and deleted=0 and SUBSTRING_INDEX(righedaa,'..',-1)>=$num_rows and SUBSTRING_INDEX(righedaa,'..',1)<=$num_rows";
...
FROM vtiger_adocdetail join vtiger_crmentity on crmid=adocdetailid
JOIN vtiger_adocmaster ON vtiger_adocmaster.adocmasterid = vtiger_adocdetail.adoctomaster
WHERE adoctomaster =$recid[0] and deleted=0 group by $fieldnameStr");
 $num_rows=$adb->num_rows($count);
//____________
...
FROM vtiger_adocdetail join vtiger_crmentity on crmid=adocdetailid
JOIN vtiger_adocmaster ON vtiger_adocmaster.adocmasterid = vtiger_adocdetail.adoctomaster
WHERE adoctomaster =$recid[0] and deleted=0 and $fieldnameStr='$grpby' ");
