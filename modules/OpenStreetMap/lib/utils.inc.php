...
function getResult($gc,$query,$idsarray=array()) {
	...
	while($row=$adb->fetch_array($result)) {
		...
		if($coord) //add item to final result
		{
			...
			if($coord->approx)
                      $approx = "<br/>".getTranslatedString('Approx','OpenStreetMap')."</br>";

//				$approx = "<br/><span style='color: grey; font-size: smaller'>".getTranslatedString('Approx','OpenStreetMap')."</span></br>";
			$content='';
			...
			$ret[$row['id']] = array(
						"name" => addslashes($row['name']).
							((empty($row['industry']) || $row['industry'] == "--None--") ? '' : "<br/>".addslashes($row['industry'])).
							((empty($row['email']) || $row['email'] == "--None--") ? '' : "<br/>".addslashes($row['email'])),
...
//		$a.= "\t,\n";
		$a.= "\t\"name\": \"{$name}\", \n";
		$a.= "\t\"city\": \"{$city}\", \n";
		$a.= "\t\"extra\": \"".str_replace(array("\r", "\r\n", "\n"), '', $result['extra'])."\", \n";
		$a.= "\t\"phone\": \"{$result['phone']}\", \n";
		$a.= "\t\"pos\": [{$result['lat']},{$result['lng']}], \n";
		$a.= "\t\"cvid\": {$result['vid']}, \n";
		$a.= "\t\"etype\": \"{$result['entitytype']}\", \n";
		$a.= "\t\"cname\": \"{$result['conname']}\" \n";
                if($i != $nr_results)
		$a.= "\t},\n";
                else $a.= "\t}\n";
	}
