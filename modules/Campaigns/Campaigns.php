...
class Campaigns extends CRMEntity {
	...
	public function get_accounts($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$button .= "<input title='".getTranslatedString('LBL_SEND_MAIL_BUTTON')."' class='slds-button slds-button--small slds-button--brand'  value='".getTranslatedString('LBL_SEND_MAIL_BUTTON');
		...
		$button .= $ahtml."<input title='".getTranslatedString('LBL_LOAD_LIST', $this_module)."' class='slds-button slds-button--small slds-button--brand'  value='";
		$button .= getTranslatedString('LBL_LOAD_LIST', $this_module)."' type='button' name='button' onclick='loadCvList(\"$related_module\",\"$id\")'>";
		$button .= '&nbsp;&nbsp;';
		$button .= "<input title='".getTranslatedString('LBL_EMPTY_LIST', $this_module)."'class='slds-button slds-button--small slds-button--brand'  value='";
	...
	public function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$button .= "<input title='".getTranslatedString('LBL_SEND_MAIL_BUTTON')."' class='slds-button slds-button--small slds-button--brand'  value='";
		...
		$button .= $lhtml."<input title='".getTranslatedString('LBL_LOAD_LIST', $this_module)."' class='slds-button slds-button--small slds-button--brand' value='";
		$button .= getTranslatedString('LBL_LOAD_LIST', $this_module)."' type='button' name='button' onclick='loadCvList(\"$related_module\",\"$id\")'>";
		$button .= '&nbsp;&nbsp;';
		$button .= "<input title='".getTranslatedString('LBL_EMPTY_LIST', $this_module)."'class='slds-button slds-button--small slds-button--brand' value='";
		$button .= getTranslatedString('LBL_EMPTY_LIST', $this_module)."' type='button' name='button' onclick='emptyCvList(\"$related_module\",\"$id\")'>";
		...
		if ($actions) {
			...
			if (in_array('SELECT', $actions) && isPermitted($related_module, 4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module, $related_module).
					"' class='slds-button slds-button--small slds-button--brand' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule".
					"&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test',".
					"'width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). ' '.
					getTranslatedString($related_module, $related_module) ."'>&nbsp;";
			}
