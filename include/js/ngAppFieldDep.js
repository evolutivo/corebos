        ...
        for(datum in data ){
            ...
            if(keyid=='56' && keyval=='si'){
                ...
                var t= keyfldname+'_values';
                 $scope[t] = [];
                 $scope[keyfldname]=[];
                 for( arr in keyoptions){
                     if(arr[2]=='selected'){
                        $scope[keyfldname].push(arr[1]);
                        $scope[t].push({value:arr[1], text:arr[0], "v":arr[2]}); 
                     }
                     else{
                        $scope[t].push({value:arr[1], text:arr[0]});
                     }
                 }
            }
  ...
  $scope.showPicklistMulti = function(fld) {
    ...
    $scope.show_logic = function(fld) {
        ...
        for(var mapid=0;mapid<mapFieldDep.length;mapid++)
        {
            ...
            var conditionResp='';
            for(var resp_key=0; resp_key<map['respfield'].length;resp_key++){
