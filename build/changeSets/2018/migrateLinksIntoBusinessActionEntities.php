		...
		<parameter></parameter>
				...
				while ($link = $adb->fetch_array($collectedLinks)) {
                                        ...
                                        'SELECT businessactionsid
                                                ...
                                                AND linkurl = ?
                                                AND reference = ?',
                                        array($link['module_list'], $link['linktype'], html_entity_decode($link['linkurl'], ENT_QUOTES, 'UTF-8'), $link['linklabel'])
                                        );
                                        if (!$adb->num_rows($linkcheck)) {
                                            ...
                                            $focusnew->column_fields['reference']=$link['linklabel'];
                                            $focusnew->column_fields['linkurl']=html_entity_decode($link['linkurl'], ENT_QUOTES, 'UTF-8');
                                            $focusnew->column_fields['moduleactions'] = $link['module_list'];
                                            $focusnew->column_fields['elementtype_action'] = $link['linktype'];
                                            $focusnew->column_fields['actions_status'] = 'Active';
